...
def _set_material(world, pos, player, tunnels, simplex):
  ...
  if start > 0.5:
    ...
    if (simplex(x, y, 6, 7) > 0.15 and mountain > 0.3) or simplex(x, y, 6, 5) > config.cave_threshold:  # cave
      world[x, y] = 'path'
      ...
      world[x, y] = 'path'
      tunnels[x, y] = True
    elif simplex(x, y, 1, 8) > config.coal_threshold and uniform() > (1 - config.coal_probability):
      world[x, y] = 'coal'
    elif simplex(x, y, 2, 6) > config.iron_threshold and uniform() > (1 - config.iron_probability):
      world[x, y] = 'iron'
      ...
      world[x, y] = 'stone'
  elif (config.sand_threshold - 0.1) < water <= config.sand_threshold and simplex(x, y, 4, 9) > -0.2:
    world[x, y] = 'sand'
  elif config.water_threshold < water:
    world[x, y] = 'water'
  else:  # grassland
    if simplex(x, y, 5, 7) > config.tree_threshold and uniform() > (1 - config.tree_probability):
      world[x, y] = 'tree'
    else:
      world[x, y] = 'grass'
...
def _set_object(world, pos, player, tunnels):
  ...
  if material not in constants.walkable:
    pass
  elif dist > config.cow_min_distance and material == 'grass' and uniform() < config.cow_spawn_probability:
    world.add(objects.Cow(world, (x, y)))
  elif dist > config.zombie_min_distance and material == 'grass' and uniform() < config.zombie_spawn_probability:
