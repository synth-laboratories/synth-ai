...
class Env(BaseClass):
  ...
  def __init__(
      ...
      reward=True, length=10000, seed=None, world_config=None):
  ...
  def _balance_chunk(self, chunk, objs):
    ...
    self._balance_object(
        chunk, objs, objects.Zombie, 'grass', 
        config.zombie_min_spawn_distance, 0, 
        config.zombie_spawn_rate, config.zombie_despawn_rate,
        lambda pos: objects.Zombie(self._world, pos, self._player),
        lambda num, space: (
            0 if space < 50 else config.zombie_max_count - (config.zombie_max_count - config.zombie_min_count) * light, 
            config.zombie_max_count - (config.zombie_max_count - config.zombie_min_count) * light))
    
    # Skeletons - static count
    self._balance_object(
        chunk, objs, objects.Skeleton, 'path', 
        config.skeleton_min_spawn_distance, config.skeleton_min_spawn_distance, 
        config.skeleton_spawn_rate, config.skeleton_despawn_rate,
    ...
    self._balance_object(
        chunk, objs, objects.Cow, 'grass', 
        config.cow_min_spawn_distance, config.cow_min_spawn_distance, 
        config.cow_spawn_rate, config.cow_despawn_rate,
