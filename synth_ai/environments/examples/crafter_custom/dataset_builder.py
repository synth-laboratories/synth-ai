...
class CrafterTask:
    ...
    global_objectives: str = "Achieve as many unique achievements as possible to maximize your score."
    shared_env_params: Dict[str, Any] = None
    
    def __post_init__(self):
        if self.shared_env_params is None:
            self.shared_env_params = {
                "render_mode": None,
                "size": (64, 64)
            }
...
class CrafterDatasetBuilder:
        ...
        for difficulty, count in instances_per_difficulty.items():
            ...
            for i in range(count):
                ...
                impetus_type = random.choices(
                    list(impetus_distribution.keys()),
                    weights=list(impetus_distribution.values())
                )[0]
                
                instance = self.create_instance(
                    difficulty=difficulty,
                    seed=seed,
                    impetus_type=impetus_type
                )
    ...
    def save_dataset(self, dataset: CrafterDataset, format: str = "json"):
        ...
        print(f"Train instances: {len(dataset.instances) - len(dataset.split_info.val_instance_ids) - len(dataset.split_info.test_instance_ids)}")
