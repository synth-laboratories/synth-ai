...
class CrafterDatasetRunner:
    ...
    def filter_instances(
        ...
        if difficulties:
            filtered = [
                ...
                if inst["metadata"]["difficulty"] in difficulties
            ]
        ...
        if impetus_types:
            filtered = [
                inst for inst in filtered
                if self._get_impetus_type(inst) in impetus_types
            ]
        ...
        if split and split_info:
            if split == "train":
                ...
                filtered = [
                    inst for inst in filtered
                    if inst["id"] in val_ids
                ]
            elif split == "test":
                test_ids = set(split_info["test_instance_ids"])
                filtered = [
                    inst for inst in filtered
                    if inst["id"] in test_ids
                ]
    ...
    def _print_summary(self, results: List[Dict[str, Any]]):
        ...
        print(f"{'Difficulty':<15} {'Count':<8} {'Success':<10} {'Avg Steps':<12} {'Avg Achievements'}")
        print("-" * 60)
        ...
        for diff in sorted(by_difficulty.keys()):
            ...
            print(f"{diff:<15} {count:<8} {success_rate:<10.1%} {avg_steps:<12.1f} {avg_achievements:.1f}")
