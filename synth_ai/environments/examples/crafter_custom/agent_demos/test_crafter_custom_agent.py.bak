...
async def run_single_episode(
    ...
    try:
        ...
        if create_resp.status_code != 200:
            print(f"  ❌ Failed to create environment: {create_resp.status_code}")
        ...
        if metadata_resp.status_code == 200:
            ...
            if config.verbose:
                print(f"\n  📊 Environment metadata for instance {instance_num}:")
...
async def evaluate_crafter_batch() -> Dict[str, Any]:
    ...
    for instance_data in instances_data:
        ...
        try:
            ...
            print(f"  ⚠️  Failed to deserialize instance {instance_data.get('id')}: {e}")
            continue
    
    # Limit to requested number of instances
    easy_task_instances = filtered_instances[:config.num_instances]
    
    print(f"  📝 Found {len(filtered_instances)} matching instances (filtered from {len(instances_data)} total)")
    
    if not easy_task_instances:
        print(f"  ❌ No instances found matching filters (difficulty={config.difficulty}, world_config={config.world_config})")
        return
    ...
    async with AsyncClient(
        ...
        for r in results:
            if isinstance(r, Exception):
                print(f"  ❌ Episode failed with exception: {r}")
                all_termination_reasons.append(f"gather_exception: {str(r)[:50]}")
                continue
            if r.get("error", False):
                print(f"  ❌ Episode failed with error: {r.get('error')}")
...
async def main():
    ...
    print(f"🎮 Custom Crafter ReAct Agent Evaluation")
    ...
    try:
        ...
        print(f"│ Assessment                │ {assessment:<41} │")
