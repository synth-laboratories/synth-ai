...
from sqlalchemy import (
    Column, Integer, String, DateTime, JSON, ForeignKey, Index, Text, Float, Boolean,
    UniqueConstraint, CheckConstraint, TypeDecorator
)
...
class Event(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("event_type IN ('cais', 'environment', 'runtime')", name='check_event_type'),
    )
...
class Message(Base):
    ...
    session_id = Column(String, ForeignKey('session_traces.session_id'), nullable=False)
    timestep_id = Column(Integer, ForeignKey('session_timesteps.id'))
    message_type = Column(String, nullable=False)  # 'user', 'assistant', 'system', 'tool_use', 'tool_result'
...
class Experiment(Base):
    ...
    systems = relationship("ExperimentalSystem", back_populates="experiment", cascade="all, delete-orphan")
