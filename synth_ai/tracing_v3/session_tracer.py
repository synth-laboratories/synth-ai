...
from .abstractions import (
    SessionTrace, SessionTimeStep, BaseEvent, SessionEventMessage, TimeRecord
)
...
class SessionTracer:
    ...
    async def start_timestep(self, 
                           step_id: str,
                           turn_number: Optional[int] = None,
                           metadata: Optional[Dict[str, Any]] = None) -> SessionTimeStep:
        ...
        # Trigger hooks
        await self.hooks.trigger("timestep_start", 
                               step=step,
    ...
    async def end_timestep(self, step_id: Optional[str] = None):
        ...
        if step_id:
            # Find specific step
            step = next((s for s in self._current_trace.session_time_steps 
        ...
        if step and step.completed_at is None:
            ...
            # Trigger hooks
            await self.hooks.trigger("timestep_end",
                                   step=step,
                                   session_id=self._current_trace.session_id)
    ...
    async def record_message(self, 
        ...
        msg = SessionEventMessage(
            ...
            time_record=TimeRecord(
                event_time=event_time or datetime.utcnow().timestamp(),
    ...
    async def get_session_history(self, limit: int = None) -> List[Dict[str, Any]]:
