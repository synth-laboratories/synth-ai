...
@pytest.mark.asyncio
class TestAsyncSQLTraceManager:
    """Test the Async SQL trace manager functionality."""
    
    @pytest_asyncio.fixture
    async def sqld_daemon(self):
        """Start sqld daemon for tests."""
        daemon = SqldDaemon(db_path=":memory:")  # Use in-memory for tests
        daemon.start(wait_for_ready=False)  # Don't wait since in-memory
        yield daemon
        daemon.stop()
    
    @pytest_asyncio.fixture
    async def db_manager(self):
        """Create an AsyncSQLTraceManager instance."""
        # Use in-memory SQLite for tests
        manager = AsyncSQLTraceManager(db_url="sqlite+aiosqlite:///:memory:")
    ...
    async def test_insert_session_trace_basic(self, db_manager):
        ...
        df = await db_manager.query_traces(
            "SELECT session_id FROM session_traces WHERE session_id = :sid",
            {"sid": session_id}
        )
    ...
    async def test_insert_session_trace_with_events(self, db_manager):
        ...
        df = await db_manager.query_traces(
            "SELECT event_type FROM events WHERE session_id = :sid",
            {"sid": session_id}
        )
