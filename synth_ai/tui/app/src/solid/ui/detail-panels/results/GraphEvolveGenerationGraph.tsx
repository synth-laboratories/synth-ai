...
import { type GenerationSummary } from "../../../../formatters/graph-evolve"
...
function formatRewardLabel(value: number, width: number): string {
  if (!Number.isFinite(value)) return "-".padEnd(width)
  const candidates = [value.toFixed(2), value.toFixed(1), value.toFixed(0)]
  const label = candidates.find((text) => text.length <= width) ?? candidates[candidates.length - 1]
...
export function GraphEvolveGenerationGraph(props: GraphEvolveGenerationGraphProps) {
  ...
  const chartLines = createMemo(() => {
    ...
    const rewardValues = slice.map((item) => item.reward)
    ...
    const points = slice.map((item, idx) => {
      const range = rewardRange === 0 ? 1 : rewardRange
      const normalized = rewardRange === 0 ? 0.5 : (item.reward - minReward) / range
    ...
    const selectedPoint = selectedGen == null
      ? null
      : points.find((point) => point.generation === selectedGen) ?? null
    if (selectedPoint && props.focused) {
      for (let row = 0; row < plotH; row += 1) {
        if (grid[row]?.[selectedPoint.x] === " ") {
          grid[row][selectedPoint.x] = ":"
        }
      }
    }
    ...
    for (let row = 0; row < plotH; row += 1) {
      let label = ""
      if (row === 0) {
        label = formatRewardLabel(maxReward, labelWidth())
      } else if (row === axisRow) {
        label = formatRewardLabel(minReward, labelWidth())
      } else if (showMid && row === midRow) {
        label = formatRewardLabel((maxReward + minReward) / 2, labelWidth())
      }
