...
(PromptLearningDetails as TextPanelComponent<DetailsPanelProps>).getLines = (props, contentWidth) => {
  const job = props.data.selectedJob
  if (!job) return wrapTextLines("No job selected.", contentWidth)
  return wrapTextLines(formatPromptLearningDetails(props.data, job), contentWidth)
}
