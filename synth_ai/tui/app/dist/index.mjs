...
}, exports_src, loadYoga, yoga_wasm_base64_esm_default, Align, BoxSizing, Dimension, Direction, Display, Edge, Errata, ExperimentalFeature, FlexDirection, Gutter, Justify, LogLevel, MeasureMode, NodeType, Overflow, PositionType, Unit, Wrap, constants, YGEnums_default, Yoga, src_default, BorderChars, BorderCharArrays, kittyKeyMap, functionalKeyMap, tildeKeyMap, metaKeyCodeRe, fnKeyRe, keyName, nonAlphanumericKeys, isShiftKey = (code) => {
  return ["[a", "[b", "[c", "[d", "[e", "[2$", "[3$", "[5$", "[6$", "[7$", "[8$", "[Z"].includes(code);
}, isCtrlKey = (code) => {
  return ["Oa", "Ob", "Oc", "Od", "Oe", "[2^", "[3^", "[5^", "[6^", "[7^", "[8^"].includes(code);
}, modifyOtherKeysRe, parseKeypress = (s = "", options = {}) => {
  let parts;
...
`) {
  ...
  return key;
}, KeyHandler, InternalKeyHandler, CSS_COLOR_NAMES, block_default, shade_default, slick_default, tiny_default, huge_default, grid_default, pallet_default, fonts, parsedFonts, TextAttributes, DebugOverlayCorner, ATTRIBUTE_BASE_MASK2 = 255, LINK_ID_SHIFT = 8, LINK_ID_PAYLOAD_MASK = 16777215, BrandedStyledText, StyledText, ESC = "\x1B", BRACKETED_PASTE_START = "\x1B[200~", BRACKETED_PASTE_END = "\x1B[201~", StdinBuffer, MouseParser, singletonCacheSymbol, envRegistry, envStore, env, TIMERS_MAP, _cachedParsers, DEFAULT_PARSERS, isUrl = (path2) => path2.startsWith("http://") || path2.startsWith("https://"), TreeSitterClient, DataPathsManager, OSC4_RESPONSE, OSC_SPECIAL_RESPONSE, pointerSize, typeSizes, primitiveKeys, typeAlignments, typeGetters, pointerPacker, pointerUnpacker, encoder, decoder, rgbaPackTransform = (rgba) => rgba ? ptr2(rgba.buffer) : null, rgbaUnpackTransform = (ptr32) => ptr32 ? RGBA.fromArray(new Float32Array(toArrayBuffer3(ptr32))) : undefined, StyledChunkStruct, HighlightStruct, LogicalCursorStruct, VisualCursorStruct, UnicodeMethodEnum, TerminalCapabilitiesStruct, EncodedCharStruct, LineInfoStruct, MeasureResultStruct, CursorStateStruct, module, targetLibPath, globalTraceSymbols = null, globalFFILogWriter = null, exitHandlerRegistered = false, LogLevel2, opentuiLibPath, opentuiLib, BrandedRenderable, LayoutEvents, RenderableEvents, BaseRenderable, yogaConfig, Renderable, RootRenderable, BrandedVNode, Capture, CapturedWritableStream, defaultKeyAliases, capture, TerminalConsoleCache, terminalConsoleCache, ConsolePosition, defaultConsoleKeybindings, DEFAULT_CONSOLE_OPTIONS, INDENT_WIDTH = 2, TerminalConsole, ANSI, KITTY_FLAG_DISAMBIGUATE = 1, KITTY_FLAG_EVENT_TYPES = 2, KITTY_FLAG_ALTERNATE_KEYS = 4, KITTY_FLAG_ALL_KEYS_AS_ESCAPES = 8, KITTY_FLAG_REPORT_TEXT = 16, MouseButton, rendererTracker, CliRenderEvents, RendererControlState, CliRenderer;
var init_index_zj0wwh9d = __esm(async () => {
  init_highlights();
  init_tree_sitter_javascript();
  init_highlights2();
  init_tree_sitter_typescript();
  init_highlights3();
  init_tree_sitter_markdown();
  init_injections();
  init_highlights4();
  init_tree_sitter_markdown_inline();
  init_highlights5();
  init_tree_sitter_zig();
...
` + d.replace(/^Error(:[^\n]*)?\n/, ""));
  ...
  KeyHandler = class KeyHandler extends EventEmitter {
    useKittyKeyboard;
  ...
  BrandedStyledText = Symbol.for("@opentui/core/StyledText");
  ...
  StdinBuffer = class StdinBuffer extends EventEmitter2 {
    buffer = "";
    timeout = null;
    timeoutMs;
    pasteMode = false;
  ...
  singletonCacheSymbol = Symbol.for("@opentui/core/singleton");
  envRegistry = singleton("env-registry", () => ({}));
  envStore = singleton("env-store", () => new EnvStore);
  env = new Proxy({}, {
    get(target, prop) {
  ...
  TreeSitterClient = class TreeSitterClient extends EventEmitter3 {
    initialized = false;
    worker;
    buffers = new Map;
    initializePromise;
  ...
  DataPathsManager = class DataPathsManager extends EventEmitter4 {
    _appName;
    _globalConfigPath;
    _globalConfigFile;
    _localConfigFile;
  ...
  primitiveKeys = Object.keys(typeSizes);
  typeAlignments = { ...typeSizes };
  typeGetters = {
  ...
  ({ pack: pointerPacker, unpack: pointerUnpacker } = primitivePackers("pointer"));
  encoder = new TextEncoder;
  decoder = new TextDecoder;
  StyledChunkStruct = defineStruct([
    ["text", "char*"],
  ...
  LogicalCursorStruct = defineStruct([
    ["row", "u32"],
    ["col", "u32"],
    ["offset", "u32"]
  ]);
  VisualCursorStruct = defineStruct([
    ["visualRow", "u32"],
    ["visualCol", "u32"],
    ["logicalRow", "u32"],
    ["logicalCol", "u32"],
  ...
  UnicodeMethodEnum = defineEnum({ wcwidth: 0, unicode: 1 }, "u8");
  TerminalCapabilitiesStruct = defineStruct([
  ...
  EncodedCharStruct = defineStruct([
    ["width", "u8"],
    ["char", "u32"]
  ]);
  LineInfoStruct = defineStruct([
    ["starts", ["u32"]],
  ...
  MeasureResultStruct = defineStruct([
    ["lineCount", "u32"],
    ["maxWidth", "u32"]
  ]);
  CursorStateStruct = defineStruct([
    ["x", "u32"],
    ["y", "u32"],
    ["visible", "bool_u8"],
    ["style", "u8"],
  ...
  module = await import(`@opentui/core-${process.platform}-${process.arch}/index.ts`);
  ...
  BrandedRenderable = Symbol.for("@opentui/core/Renderable");
  ((LayoutEvents2) => {
  ...
  BaseRenderable = class BaseRenderable extends EventEmitter6 {
  ...
  yogaConfig = src_default.Config.create();
  yogaConfig.setUseWebDefaults(false);
  yogaConfig.setPointScaleFactor(1);
  Renderable = class Renderable extends BaseRenderable {
  ...
  RootRenderable = class RootRenderable extends Renderable {
    renderList = [];
    constructor(ctx) {
  ...
  BrandedVNode = Symbol.for("@opentui/core/VNode");
  Capture = class Capture extends EventEmitter7 {
    output = [];
    constructor() {
      super();
    }
  ...
  CapturedWritableStream = class CapturedWritableStream extends Writable {
    stream;
    capture;
    isTTY = true;
  ...
  capture = singleton("ConsoleCapture", () => new Capture);
  registerEnvVar({
  ...
  TerminalConsoleCache = class TerminalConsoleCache extends EventEmitter8 {
    _cachedLogs = [];
    MAX_CACHE_SIZE = 1000;
  ...
  terminalConsoleCache = singleton("TerminalConsoleCache", () => {
  ...
  TerminalConsole = class TerminalConsole extends EventEmitter8 {
    isVisible = false;
    isFocused = false;
    renderer;
    keyHandler;
...
`);
  ...
  rendererTracker = singleton("RendererTracker", () => {
  ...
  CliRenderer = class CliRenderer extends EventEmitter9 {
...
var init_core = __esm(async () => {
  ...
  BoxRenderable = class BoxRenderable extends Renderable {
    _backgroundColor;
    _border;
    _borderStyle;
    _borderColor;
  ...
  TextBufferRenderable = class TextBufferRenderable extends Renderable {
    selectable = true;
    _defaultFg;
    _defaultBg;
    _defaultAttributes;
  ...
  BrandedTextNodeRenderable = Symbol.for("@opentui/core/TextNodeRenderable");
  TextNodeRenderable = class TextNodeRenderable extends BaseRenderable {
  ...
  RootTextNodeRenderable = class RootTextNodeRenderable extends TextNodeRenderable {
    ctx;
    textParent;
...
`)) {
  ...
  TextRenderable = class TextRenderable extends TextBufferRenderable {
    _text;
  ...
  InputRenderable = class InputRenderable extends Renderable {
    _focusable = true;
    _value = "";
    _cursorPosition = 0;
    _placeholder;
  ...
  EditBufferRenderable = class EditBufferRenderable extends Renderable {
    _focusable = true;
    selectable = true;
    _textColor;
    _backgroundColor;
...
async function loadPersistedSettings(deps) {
  ...
  try {
    ...
    const localKey = typeof localKeyFromFile === "string" && localKeyFromFile.trim() ? localKeyFromFile.trim() : (process.env.SYNTH_API_KEY || process.env.SYNTH_TUI_API_KEY_LOCAL || "").trim();
    if (typeof prodKey === "string" && prodKey.trim())
      setBackendKey("prod", prodKey.trim());
    if (typeof devKey === "string" && devKey.trim())
      setBackendKey("dev", devKey.trim());
    if (localKey)
...
`);
    ...
    if (selected) {
      let key = backendKeys2[selected.id];
...
function loadEnvFiles() {
  ...
  const envFiles = [
    join2(cwd, ".env"),
    join2(cwd, ".env.local"),
    join2(cwd, "backend", ".env")
  ];
  const parentDir = join2(cwd, "..");
  envFiles.push(join2(parentDir, ".env"));
...
loadEnvFiles();
init_app().then(() => exports_app).then(({ runApp: runApp2 }) => {
  runApp2().catch((err) => {
