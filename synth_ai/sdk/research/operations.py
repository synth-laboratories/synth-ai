...
RESEARCH_OPERATIONS = {
    ...
    for operation in (
        ...
        _operation(
            "create_visual", HttpMethod.POST, "/smr/visuals", mutation=True, idempotent=False
        ),
        _operation(
...
# Renamed operations retain the same HTTP method and path in the current
# backend contract. Removed endpoints are rejected before transport execution.
...
RETIRED_RESEARCH_OPERATION_IDS = frozenset(
    [
...
def research_operation(operation_id: str) -> OperationMetadata:
    if operation_id in RETIRED_RESEARCH_OPERATION_IDS:
        raise ValueError(
            f"unsupported Research operation_id {operation_id!r}: "
            "endpoint is absent from the current backend public contract"
        )
    operation_id = RESEARCH_OPERATION_ALIASES.get(operation_id, operation_id)
    try:
