...
class InternAsyncEnsureRequest(_StrictContract):
    ...
    # None omits the field on the wire (``to_wire`` elides nulls) so older
    # strict backends keep accepting ensure requests; the backend default (0)
    # refuses immediately with a typed readiness report.
    factory_ready_wait_seconds: int | None = Field(default=None, ge=0, le=60)
