...
support a small TOML wrapper that points at an ADASTaskSet JSON file plus a few
optimization knobs.
...
```toml
[graph]
dataset = "my_tasks.json"          # required (path to ADASTaskSet JSON)
...
def validate_graph_job_section(
    ...
    *,
    ...
    if not dataset_ref or not isinstance(dataset_ref, str):
        ...
        try:
            ...
            errors.append(
                {
                    "field": "graph.dataset",
                    "error": f"Invalid ADASTaskSet JSON: {e}",
                }
            )
            dataset = None
    ...
    if dataset is not None:
        try:
            validate_adas_job_config(config, dataset)
        except ADASValidationError as e:
...
def validate_graph_job_payload(payload: Dict[str, Any]) -> None:
    ...
    dataset: Optional[ADASTaskSet]
    if isinstance(dataset_raw, ADASTaskSet):
        dataset = dataset_raw
    elif isinstance(dataset_raw, dict):
        try:
            dataset = ADASTaskSet.model_validate(dataset_raw)
        except Exception as e:
            errors.append({"field": "dataset", "error": f"Invalid ADASTaskSet: {e}"})
            dataset = None
    else:
