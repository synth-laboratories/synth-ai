...
def prepare_for_openai(model: str | None, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if synth_ai_py is not None and hasattr(synth_ai_py, "localapi_prepare_for_openai"):
...
def prepare_for_groq(model: str | None, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if synth_ai_py is not None and hasattr(synth_ai_py, "localapi_prepare_for_groq"):
...
def normalize_response_format_for_groq(model: str | None, payload: dict[str, Any]) -> None:
    ...
    if synth_ai_py is not None and hasattr(
        synth_ai_py, "localapi_normalize_response_format_for_groq"
    ):
...
def inject_system_hint(payload: dict[str, Any], hint: str) -> dict[str, Any]:
    ...
    if synth_ai_py is not None and hasattr(synth_ai_py, "localapi_inject_system_hint"):
...
def extract_message_text(message: Any) -> str:
    ...
    if synth_ai_py is not None and hasattr(synth_ai_py, "localapi_extract_message_text"):
...
def parse_tool_call_from_text(text: str) -> tuple[list[str], str]:
    ...
    if synth_ai_py is not None and hasattr(synth_ai_py, "localapi_parse_tool_call_from_text"):
...
def synthesize_tool_call_if_missing(
    ...
    if synth_ai_py is not None and hasattr(synth_ai_py, "localapi_synthesize_tool_call_if_missing"):
        return synth_ai_py.localapi_synthesize_tool_call_if_missing(
