...
class PromptExtractor:
    ...
    def extract_from_candidate(
        ...
        elif hasattr(candidate, "object") and hasattr(
            candidate, "payload_kind"
        ):  # AttemptedCandidate-like
        ...
        if isinstance(data, dict):
            ...
            if "messages" in data:
                return PromptExtractor._extract_from_pattern({"messages": data["messages"]})
            if "sections" in data:
    ...
    @staticmethod
    def _extract_from_pattern_obj(pattern: Any) -> Optional[ExtractedPrompt]:
        """Extract from PromptPattern dataclass."""
        if not hasattr(pattern, "messages") or not pattern.messages:
            return None
        sections_dict = []
        text_parts = []
        role = "system"
        for message in pattern.messages:
            content = getattr(message, "pattern", None) or getattr(message, "content", "")
            msg_role = getattr(message, "role", "system")
            if content:
                sections_dict.append(
                    {"role": msg_role, "content": content, "name": getattr(message, "name", None)}
                )
    ...
    @staticmethod
    def _extract_from_pattern(obj: Dict[str, Any]) -> Optional[ExtractedPrompt]:
        """Extract from PromptPattern structure."""
        ...
        for msg in messages:
            ...
            content = msg.get("pattern") or msg.get("content") or ""
            if content:
                sections.append({"role": msg_role, "content": content, "name": msg.get("name")})
                text_parts.append(str(content))
                if role == "system":
                    role = str(msg_role)
        if not text_parts:
