...
def build_placeholder_dataset() -> GraphEvolveTaskSet:
    if synth_ai_py is not None and hasattr(synth_ai_py, "build_graph_evolve_placeholder_dataset"):
        payload = synth_ai_py.build_graph_evolve_placeholder_dataset()
        return GraphEvolveTaskSet.model_validate(payload)
    return GraphEvolveTaskSet.model_validate(
        {
            "metadata": {"name": "Placeholder Graph Evolve Dataset"},
            "tasks": [
                {
                    "id": "task-1",
