...
def display_prompt_learning_summary(
    *,
    ...
    try:
        # Fetch job status
        job_url = f"{backend_base}/policy-optimization/online/jobs/{job_id}"
        ...
        # Fetch events
        events_url = f"{backend_base}/policy-optimization/online/jobs/{job_id}/events?limit=1000"
        ...
        rows.append(
            ("Cost", f"Policy: {cost_policy} | Proposal: {cost_proposal} | Total: {cost_total}")
        )
        ...
        if algorithm == "mipro":
            ...
            if baseline_acc is not None and results and len(results) > 0:
                ...
                if result_acc is not None:
                    ...
                    rows.append(
                        ("Candidate 1", f"Accuracy: {result_acc:.4f} (Δ{delta:+.4f} vs baseline)")
                    )
        ...
        if show_curve and optimization_curve:
            from synth_ai.sdk.optimization._impl.plotting import plot_optimization_curve
