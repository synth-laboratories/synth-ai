...
The table is complete: every code an Index route can return is named here, and
the release parity gate fails when the backend adds, drops or renames one. The
only codes deliberately absent are operator acceptance-run codes, which a
customer credential cannot provoke (see ``OPERATOR_ONLY_ERROR_CODES``).
"""
