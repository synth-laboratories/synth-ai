"""Shared helper utilities for Container containers."""
...
def normalize_chat_completion_url(url: str) -> str:
    ...
    return synth_ai_py.container_normalize_chat_completion_url(url)


def get_default_max_completion_tokens(model_name: str) -> int:
    """Get default max_completion_tokens based on model name."""
    return int(synth_ai_py.container_get_default_max_completion_tokens(model_name))
...
def extract_api_key(
    ...
    return synth_ai_py.container_extract_api_key(headers, policy_config, default_env_keys)
...
def parse_tool_calls_from_response(
    ...
    return synth_ai_py.container_parse_tool_calls_from_response(response_json, expected_tool_name)
...
async def call_chat_completion_api(
    ...
    *,
    ...
    """Unified chat completion API caller with common Container logic."""
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=f"Proxy POST failed: {exc}") from exc
