...
container code to work both locally and with Harbor deployments.

Example:
    >>> from synth_ai.sdk.container import ContainerConfig
    >>> from synth_ai.sdk.container.harbor.adapter import HarborExecutionBackend
...
class HarborExecutionBackend:
    ...
    allowing it to be used as the `rollout` parameter in ContainerConfig.
    ...
    def _transform_response(
        ...
        # Extract reward and optional rollout telemetry from various possible formats
        reward = 0.0
        details: dict[str, Any] = {}
        source_metrics: dict[str, Any] = {}

        if isinstance(harbor_response.get("reward_info"), dict):
            source_metrics = harbor_response["reward_info"]
        elif isinstance(harbor_response.get("metrics"), dict):
            source_metrics = harbor_response["metrics"]

        raw_reward = source_metrics.get("outcome_reward", 0.0)
        if isinstance(raw_reward, (int, float)):
            reward = float(raw_reward)

        raw_details = source_metrics.get("details")
        ...
        metrics_payload: dict[str, Any] = {"outcome_reward": reward, "details": details}
        for key in (
            "event_rewards",
            "outcome_objectives",
            "event_objectives",
        ...
        return RolloutResponse(
            ...
            reward_info=RolloutMetrics(**metrics_payload),
    ...
    async def __call__(self, request: RolloutRequest, fastapi_request: Request) -> RolloutResponse:
        ...
        `rollout` parameter in ContainerConfig.
...
def create_harbor_rollout_executor(
    ...
    """Create a Harbor rollout executor for use in ContainerConfig.
    ...
    Example:
        >>> from synth_ai.sdk.container import ContainerConfig
        >>> from synth_ai.sdk.container.harbor.adapter import create_harbor_rollout_executor
        >>>
        >>> config = ContainerConfig(
        ...     app_id="my_task",
        ...     name="My Task",
