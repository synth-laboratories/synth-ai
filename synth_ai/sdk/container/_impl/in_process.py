"""In-process container support for local development and demos."""
...
_registered_instances: set["InProcessContainer"] = set()
    ...
    *,
    ...
    for attempt in range(max_retries):
        try:
            ...
            if resolved_ip:
                loop = asyncio.get_event_loop()
                # Capture resolved_ip in closure
                current_resolved_ip = resolved_ip

                # Use curl with --resolve to bypass local DNS while maintaining proper SNI
                async def curl_check(
                    path: str, _loop: Any = loop, _resolved_ip: str = current_resolved_ip
                ) -> int:
                    try:
                        curl_cmd = [
                            "curl",
...
async def _verify_preconfigured_url_ready(
    ...
    *,
    ...
    Args:
        ...
        api_key: API key for container authentication
...
class InProcessContainer:
    ...
    (Alias: also known as "container" in older documentation)
    ...
    def __init__(
        ...
        *,
        ...
        container_path: Optional[Path | str] = None,
        port: int = 8114,
        host: str = "127.0.0.1",
        ...
        on_start: Optional[Callable[["InProcessContainer"], None]] = None,
        on_stop: Optional[Callable[["InProcessContainer"], None]] = None,
    ):
        ...
        Args:
            ...
            on_start: Optional callback called when container starts (receives self)
            on_stop: Optional callback called when container stops (receives self)

        Raises:
            ValueError: If multiple or no input methods provided, or invalid parameters
            FileNotFoundError: If container_path doesn't exist
        """
        # Validate: exactly one input method
        inputs = [app, config, config_factory, container_path]
        if sum(x is not None for x in inputs) != 1:
            raise ValueError(
                "Must provide exactly one of: app, config, config_factory, or container_path"
            )
        ...
        # Validate container_path if provided
        if container_path:
            path = Path(container_path)
            if not path.exists():
                raise FileNotFoundError(f"Container path does not exist: {container_path}")
            if path.suffix != ".py":
                raise ValueError(f"Container path must be a .py file, got {container_path}")

        self._app_input = app
        self._config = config
        self._config_factory = config_factory
        self._container_path = Path(container_path) if container_path else None
        ...
        self._tunnel_handle: Optional[TunneledContainer] = None
        ...
        self.container_worker_token: Optional[str] = None
    ...
    async def __aenter__(self) -> "InProcessContainer":
        ...
        if self._app_input:
            ...
            # ContainerConfig - create app from it
            self._app = create_container(self._config)  # type: ignore[assignment]

        elif self._config_factory:
            # Callable - call it to get config, then create app
            config = self._config_factory()
            self._app = create_container(config)  # type: ignore[assignment]

        elif self._container_path:
            # File path - load module and extract app
            from synth_ai.core.utils.paths import temporary_import_paths

            with temporary_import_paths(self._container_path, REPO_ROOT):
                module = load_module(
                    self._container_path,
                    f"_inprocess_{self._container_path.stem}_{id(self)}",
                )
            ...
            try:
                ...
                if build_config and callable(build_config):
                    ...
                    from synth_ai.sdk.container._impl.apps import registry

                    app_id = getattr(module, "APP_ID", None) or self._container_path.stem
                    ...
                    if entry and entry.config_factory:
                        ...
                        raise RuntimeError(
                            f"Container at {self._container_path} must expose either:\n"
                            f"  - An ASGI app via `app = FastAPI(...)` or factory function\n"
                            f"  - A `build_config()` function that returns ContainerConfig\n"
                            f"  - Be registered with register_container() or register_container()"
                        ) from None

        # Resolve the container API key before starting the server.
        #
        # The Container enforces auth via `ENVIRONMENT_API_KEY`. If we allow the server
        ...
        config = uvicorn.Config(
            ...
            reload=False,
            log_level="info",
            # In-process containers can receive high request volumes (e.g. rollouts).
        ...
        if mode == "preconfigured":
            ...
            if backend == TunnelBackend.SynthTunnel:
                self.container_worker_token = self._tunnel_handle.worker_token
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        logger.info("Stopping in-process container...")
    ...
    def _get_api_key(self) -> str:
        ...
        # - Tunnel agents that must authenticate to the local container
        try:
