...
class RubricBundle(BaseModel):
    """Rubrics advertised by the container.
...
class _ServiceInfo(BaseModel):
    """Metadata about the running container service."""
...
class ContainerConfig:
    """Declarative configuration describing a Container.

    A ContainerConfig defines all the components needed to create a container:
    ...
    Example:
        ...
        ...     )
        >>>
        >>> app = create_container(build_config())
        >>> # app is a FastAPI instance ready to run

    Attributes:
        app_id: Unique identifier for this container
        name: Human-readable name
        description: Description of what this container does
    ...
    ensure_container_auth: bool = True
    ...
    def clone(self) -> ContainerConfig:
        ...
        return ContainerConfig(
            ...
            ensure_container_auth=self.ensure_container_auth,
...
class ContainerConfig(ContainerConfig):
    """Alias for ContainerConfig with Container naming."""
...
def _build_proxy_routes(
    app: FastAPI, config: ContainerConfig, auth_dependency: Callable[[Request], None]
) -> None:
    proxy = config.proxy
    if not proxy:
        return
...
def _auth_dependency_factory(config: ContainerConfig) -> Callable[[Request], None]:
...
def create_container(config: ContainerConfig) -> FastAPI:
    """Create a FastAPI application from a ContainerConfig.

    This function builds a complete FastAPI application that implements the Synth AI
    container contract. The resulting app includes:
    ...
    Args:
        config: ContainerConfig describing the container
    ...
    log_info("create_container invoked", ctx=ctx)
    cfg = config.clone()
...
def run_container(
    ...
    **_kwargs: Any,
    ...
    """Run a container locally with uvicorn.

    This is a convenience function that creates a container from a config factory
    and runs it with uvicorn. It handles service registration for tunnel management.

    Args:
        config_factory: Function that returns a ContainerConfig
        host: Host to bind to (default: "0.0.0.0")
        port: Port to listen on (default: 8001)
        reload: Enable auto-reload on file changes (default: False)
    Example:
        >>> from synth_ai.sdk.container._impl.server import run_container
        >>> from my_container import build_config
        >>>
        >>> run_container(
        ...     build_config,
        ...     host="127.0.0.1",
    ...
    Raises:
        ...
        TypeError: If config_factory doesn't return ContainerConfig
    """
    ...
    if not isinstance(config, ContainerConfig):  # type: ignore[arg-type]
        raise TypeError(
            f"Container config_factory must return ContainerConfig, got {type(config).__name__}"
        )
    ...
    try:
        ...
        raise RuntimeError("uvicorn must be installed to run the container locally") from exc
    ...
    try:
        ...
        try:
            from synth_ai.core.tunnels.service_records import remove_service_record
...
def run_server_background(
    ...
    For manual control over container lifecycle. If you want automatic
    tunnel management, use InProcessContainer instead.
