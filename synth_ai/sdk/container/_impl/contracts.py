...
class ContainerEndpoints:
    """Required Container endpoints used by RL trainers and clients.

    Containers run as lightweight HTTP services (often on Modal) that expose these
    standard endpoints. Additional endpoints (proxies, debug routes) may be added
    by individual containers as needed.
    """
...
@dataclass(frozen=True)
class ContainerEndpoints(ContainerEndpoints):
    """Alias for ContainerEndpoints with Container naming."""


# --- Unified rollout schema used by Container services and SDK utilities ---
...
class RolloutSafetyConfig(BaseModel):
    ...
    Note: max_time_s is set but enforcement depends on the container implementation.
    """

    # NOTE: synth_ai_py.container_build_rollout_response currently expects this to
...
class RolloutRequest(BaseModel):
    ...
    - `context_overrides`: Optional list of context overrides (AGENTS.md, skills, scripts, env vars)
      to apply before running the agent. Container is responsible for applying these.
    ...
    context_overrides: list[ContextOverride] | None = Field(
        ...
        "for unified optimization. Container applies these before running the agent.",
    )
...
class TaskInfo(_ExtraAllowModel):
    """Static metadata describing the capabilities of a Container task."""
