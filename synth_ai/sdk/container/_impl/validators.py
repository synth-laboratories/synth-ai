...
from synth_ai.sdk.container._impl.contracts import ContainerEndpoints  # type: ignore[attr-defined]


def validate_rollout_response_for_rl(
    response_data: dict[str, Any], *, warn_only: bool = False
) -> list[str]:
    """Validate that a container rollout response has required fields for RL training.
    ...
    if synth_ai_py is not None and hasattr(
        synth_ai_py, "container_validate_rollout_response_for_rl"
    ):
        issues = synth_ai_py.container_validate_rollout_response_for_rl(response_data)
    else:
        issues = []
    ...
    if issues and not warn_only:
        error_msg = "Container response validation failed for RL training:\n" + "\n".join(
...
def normalize_inference_url(
    ...
    if synth_ai_py is not None and hasattr(synth_ai_py, "container_normalize_inference_url"):
        return synth_ai_py.container_normalize_inference_url(url, default)
...
def validate_container_url(url: str | None) -> str:
    """Validate and normalize a container URL."""

    if not url:
        raise ValueError("Container URL is required")
    if synth_ai_py is not None and hasattr(synth_ai_py, "validate_container_url"):
        return synth_ai_py.validate_container_url(url)
    ...
    if not parsed.netloc:
        raise ValueError("Container URL is invalid")
...
async def validate_container_endpoint(
    url: str,
    api_key: str | None = None,
    min_instances: int = 10,
    verbose: bool = False,
) -> tuple[bool, dict[str, Any]]:
    """Validate a container deployment.
