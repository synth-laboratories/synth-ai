"""Internal container client.

Public API: Use `synth_ai.sdk.container.client` instead.
"""
...
def _require_rust() -> Any:
    if synth_ai_py is None:
        raise RuntimeError("synth_ai_py is required for Container. Install rust bindings.")
    return synth_ai_py
...
class ContainerClient:
    def __init__(
        ...
        *,
        ...
        self.env = _ContainerEnvironmentClient(self)
        rust = _require_rust()
        self._rust_client = rust.ContainerClient(self.base_url, self.api_key, int(timeout))
...
class ContainerClient(ContainerClient):
    """Alias for ContainerClient with Container naming."""


class _ContainerEnvironmentClient:
