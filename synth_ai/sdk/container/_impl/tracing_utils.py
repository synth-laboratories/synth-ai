"""Utilities for wiring tracing_v3 into containers."""
...
def tracing_env_enabled(default: bool = False) -> bool:
    """Return True when tracing is enabled for containers via environment variable."""

    return synth_ai_py.container_tracing_env_enabled(default)


def resolve_tracing_db_url() -> str | None:
    """Resolve tracing database URL using centralized logic."""

    return synth_ai_py.container_resolve_tracing_db_url()
...
def resolve_sft_output_dir() -> str | None:
    ...
    return synth_ai_py.container_resolve_sft_output_dir()


def unique_sft_path(base_dir: str, *, run_id: str):
    """Return a unique JSONL path for an SFT record batch."""

    return synth_ai_py.container_unique_sft_path(base_dir, run_id)
