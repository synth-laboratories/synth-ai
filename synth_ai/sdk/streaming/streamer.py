    ...
    async def _stream_events_sse(self, sse_url: str) -> AsyncIterator[StreamMessage]:
        ...
        async with aiohttp.ClientSession(headers=headers, timeout=timeout) as session:
            async with session.get(url) as resp:
                ...
                print(
                    f"[DEBUG] SSE stream connected to {url}, status={resp.status}", file=sys.stderr
                )
                buffer = ""
                event_count = 0
                ...
                async for chunk in resp.content.iter_chunked(8192):
                    ...
                    if (
                        ...
                        print(
                            f"[DEBUG] WARNING: No events received via SSE for 10s after connection. Backend may not be publishing to Redis (check SSE_USE_REDIS env var).",
                            file=sys.stderr,
                        )
    ...
    async def _poll_events(self, http: AsyncHttpClient) -> list[StreamMessage]:
        ...
        for path in self._event_paths:
            ...
            try:
                ...
                print(
                    f"[DEBUG] Polling {path} with since_seq={since}, limit={limit}", file=sys.stderr
                )
                print(
                ...
                if "404" in error_str and (
                    "graphgen" in path.lower() or "prompt-learning" in path.lower()
                ):
