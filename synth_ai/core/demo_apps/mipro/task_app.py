    ...
    def _load_split(self, split: str):
        ...
        if split not in self._cache:
            try:
                ...
                if (
                    self._label_names is None
                    and label_feature is not None
                    and hasattr(label_feature, "names")
                ):
...
async def call_chat_completion(
    ...
    if is_provider_host:
        ...
        raise HTTPException(
            status_code=502, detail=f"Direct provider URL not allowed for policy: {route_base}"
        )
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        with contextlib.suppress(Exception):
            ...
            if "X-API-Key" in headers:
                print(
                    f"[TASK_APP] ✅ X-API-Key IS in headers (len={len(headers['X-API-Key'])})",
                    flush=True,
                )
            else:
...
async def rollout_executor(request: RolloutRequest, fastapi_request: Request) -> RolloutResponse:
    ...
    with contextlib.suppress(Exception):
        ...
        print(
            f"[TASK_APP] INBOUND_ROLLOUT: cid={request.trace_correlation_id} seed={request.env.seed} env={request.env.env_name} "
    ...
    if tool_calls:
        ...
        print(
            f"[TASK_APP] CONTENT_FALLBACK_INTENT: {predicted_intent} text_len={len(response_text or '')}",
            flush=True,
        )
    ...
    is_correct = predicted_intent.lower().replace("_", " ") == expected_intent.lower().replace(
        "_", " "
    )
    ...
    with contextlib.suppress(Exception):
        print(
            f"[BANKING77_ROLLOUT] cid={request.trace_correlation_id} split={sample['split']} "
...
def provide_task_instances(dataset: Banking77Dataset, seeds: Sequence[int]) -> Iterable[TaskInfo]:
    ...
    base_dataset = (
        ...
        if hasattr(base_info.dataset, "model_dump")
        else dict(base_info.dataset)
    )
    base_metadata = (
        base_info.task_metadata.model_dump()
        if hasattr(base_info.task_metadata, "model_dump")
        else dict(base_info.task_metadata)
    )
    for seed in seeds:
