...
from synth_ai.core.env_utils import mask_str
...
if TYPE_CHECKING:
    from synth_ai.core.demo_apps.demo_task_apps.core import DemoEnv
...
def _get_default_task_app_secret_name():
    """Lazy import to avoid circular dependency."""
    from synth_ai.core.demo_apps.demo_task_apps.core import DEFAULT_TASK_APP_SECRET_NAME
...
def ensure_task_app_ready(env: "DemoEnv", synth_key: str, *, label: str) -> "DemoEnv":
    ...
    demo_core.persist_task_url(
        task_url, name=app_name if requires_modal_name else None, path=persist_path
    )
    if synth_key:
        os.environ["SYNTH_API_KEY"] = synth_key

    openai_key = (
        os.environ.get("OPENAI_API_KEY") or str(user_config_map.get("OPENAI_API_KEY") or "")
    ).strip()
    if openai_key:
