...
def validate_eval_options(options: dict[str, object]) -> dict[str, object]:
    ...
    normalized["seeds"] = _parse_seeds(seeds)

    metadata = normalized.get("metadata") or []
    normalized["metadata"] = _parse_metadata(metadata)
    ...
    if "ops" in normalized:
        normalized["ops"] = _parse_ops(normalized.get("ops") or "")

    if "poll" in normalized and normalized["poll"] not in (None, ""):
        normalized["poll"] = float(normalized["poll"])
