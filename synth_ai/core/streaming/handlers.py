...
class OptimizationStreamHandler(StreamHandler):
    """Base handler with support for typed OpenResponses-aligned events.

    Subclasses can override handle_job_event() and handle_candidate_event()
    to process typed events. The default handle() method will route to these
    ...
    Example:
        >>> class MyHandler(OptimizationStreamHandler):
        ...     def handle_job_event(self, message, event):
        ...         if event.event_type == JobEventType.JOB_COMPLETED:
        ...             print(f"Job {event.job_id} completed!")
        ...
        ...     def handle_candidate_event(self, message, event):
        ...         print(f"Candidate {event.candidate_id} event: {event.event_type}")
    """

    def handle(self, message: StreamMessage) -> None:
        """Route messages to typed or raw handlers based on event type."""
        ...
        if message.stream_type is StreamType.EVENTS:
            ...
            if typed_event is not None:
                ...
                elif isinstance(typed_event, JobEvent):
                    self.handle_job_event(message, typed_event)
                else:
                    # BaseJobEvent - treat as job event
                    self.handle_job_event(message, typed_event)
                return

        # Fall through to raw handling for non-typed events
        self.handle_raw_message(message)

    def handle_job_event(self, message: StreamMessage, event: JobEvent | BaseJobEvent) -> None:
        """Handle a typed job lifecycle event.

        Override this method to process job events (in_progress, completed, failed, cancelled).
        Default implementation falls through to handle_raw_message.

        Args:
            message: The original StreamMessage
            event: The typed JobEvent (or BaseJobEvent for unknown subtypes)
        """
        self.handle_raw_message(message)

    def handle_candidate_event(self, message: StreamMessage, event: CandidateEvent) -> None:
        """Handle a typed candidate lifecycle event.

        Override this method to process candidate events (added, completed).
        Default implementation falls through to handle_raw_message.

        Args:
            message: The original StreamMessage
            event: The typed CandidateEvent
        """
        self.handle_raw_message(message)

    def handle_raw_message(self, message: StreamMessage) -> None:
        """Handle a message without typed event support.

        Override this method to process messages that don't have typed events
        (metrics, status, timeline) or events with unrecognized types.
...
class GraphGenHandler(StreamHandler):
    ...
    via a factory; by default we choose a policy-optimization handler for GEPA/MIPRO and
    ...
    def __init__(
        ...
        *,
        ...
        # Options for the default policy-optimization delegate
    ...
    def _is_prompt_learning_type(job_type: str | None) -> bool:
        """Return True if the child job type should use policy-optimization formatting."""
        return job_type in {
            "gepa",
            "mipro",
            "prompt_learning",
...
class GraphEvolveHandler(GraphGenHandler):
    """Alias for GraphGenHandler (Graph Evolve naming)."""
