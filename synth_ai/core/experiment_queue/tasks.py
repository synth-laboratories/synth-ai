...
def _extract_backend_job_id(output: str) -> str | None:
    ...
    if match:
        ...
        assert job_id.startswith("pl_"), (
            f"Extracted job_id doesn't match expected format 'pl_*': {job_id}"
        )
...
def _poll_backend_progress(
    ...
    poller_logger = logging.getLogger(f"synth_ai.core.experiment_queue.poller.{backend_job_id}")
    ...
    poller_logger.info(
        "📡 Starting progress poller for backend job %s (URL: %s)", backend_job_id, url
    )
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            try:
                ...
                is_pool_exhausted = (
                    "connection" in error_str or "timeout" in error_str or "refused" in error_str
                )
                if is_pool_exhausted:
                    ...
                    print(
                        "⚠️  Check DB_POOL_SIZE and DB_MAX_OVERFLOW environment variables",
                        flush=True,
                    )
                    print("=" * 100, flush=True)
            ...
            poller_logger.info(
                "API response: status=%d, content_length=%d", resp.status_code, len(resp.content)
            )
            ...
            if resp.status_code not in (200, 201):
                ...
                if is_pool_exhausted:
                    ...
                    print("=" * 100, flush=True)
                    print(
                        "🔥🔥🔥 CONNECTION POOL EXHAUSTION DETECTED (POLLER HTTP ERROR) 🔥🔥🔥",
                        flush=True,
                    )
                    print("=" * 100, flush=True)
                    print(f"Backend Job ID: {backend_job_id}", flush=True)
            ...
            if resp.status_code == 200:
                ...
                try:
                    ...
                    for idx, event in enumerate(events_response.events):
                        ...
                        if event.type == "prompt.learning.progress":
                            ...
                            if (
                                ...
                                and progress_start_time is None
                            ):
                            ...
                            if event.data and isinstance(event.data, dict):
                                ...
                                if phase == "validation":
                                    ...
                                    if "validation_candidate" in event.data:
                                        custom_fields["validation_candidate"] = event.data[
                                            "validation_candidate"
                                        ]
                                    if "validation_total" in event.data:
                                        custom_fields["validation_total"] = event.data[
                                            "validation_total"
                                        ]
                            ...
                            if has_progress:
                                ...
                                assert hasattr(status_tracker, "update"), (
                                    "status_tracker missing update method"
                                )
                                assert hasattr(status_tracker, "job_id"), (
                                    "status_tracker missing job_id"
                                )
                            ...
                            if rollouts_completed is not None:
                                if (
                                    last_rollouts_completed is None
                                    or rollouts_completed != last_rollouts_completed
                                ):
                    ...
                    if events_received == 0:
                        ...
                        poller_logger.info(
                            "Progress poller heartbeat for job %s (no new events, last_seq=%d, consecutive_no_events=%d)",
                            backend_job_id,
                            last_seq,
                            no_event_count,
                        )
                        ...
                        if (
                            hasattr(_poll_backend_progress, "_no_event_polls")
                            and backend_job_id in _poll_backend_progress._no_event_polls
                        ):  # type: ignore[attr-defined]
                            _poll_backend_progress._no_event_polls[backend_job_id] = 0  # type: ignore[attr-defined]

                        event_types_str = ", ".join(
                            f"{k}:{v}" for k, v in sorted(event_types_seen.items())
                        )
                        poller_logger.info(
                ...
                poller_logger.warning(
                    "Backend job %s not found (404), stopping poller", backend_job_id
                )
                break
            ...
            backoff_seconds = min(
                base_poll_interval * (2 ** min(consecutive_timeouts - 1, 4)), 60.0
            )  # Max 60s backoff
            poller_logger.warning(
            ...
            consecutive_timeouts = 0
            poller_logger.error(
                "Progress poller error for job %s: %s", backend_job_id, e, exc_info=True
            )
    ...
    with session_scope() as session:
                        ...
                        if resp.status_code == 200:
                            ...
                            if backend_stats:
                                ...
                                status_update = {
                                    ...
                                    "optimization_rollouts_executed": backend_stats.get(
                                        "optimization_rollouts_executed"
                                    ),
                                    "validation_rollouts_executed": backend_stats.get(
                                        "validation_rollouts_executed"
                                    ),
                                    "optimization_trials_evaluated": backend_stats.get(
                                        "optimization_trials_evaluated"
                                    ),
                                    "validation_trials_evaluated": backend_stats.get(
                                        "validation_trials_evaluated"
                                    ),
                                    ...
                                    "best_score": backend_stats.get("best_score")
                                    or backend_stats.get("best_validation_score"),
                                    "total_time_seconds": backend_stats.get("total_time_seconds"),
                                    "eval_seeds_n": backend_stats.get("eval_seeds_n"),
                                    "transformations_evaluated": backend_stats.get(
                                        "transformations_evaluated"
                                    ),
                                }
                                # Remove None values
                                status_update = {
                                    k: v for k, v in status_update.items() if v is not None
                                }
                                # ✅ ADD: Assertion to ensure we have at least some stats
                                assert len(status_update) > 0, (
                                    f"status_update must not be empty for job {job_id}"
                                )
                                if status_update:
        ...
        if experiment:
            ...
            if remaining == 0:
                ...
                all_failed = all(job.status == ExperimentJobStatus.FAILED for job in all_jobs)
                if all_failed:
                    experiment.status = ExperimentStatus.FAILED
                    experiment.error = all_jobs[0].error if all_jobs else "All jobs failed"
                else:
...
@celery_app.task(bind=True, name="synth_ai.core.experiment_queue.run_experiment_job")
def run_experiment_job(self, job_id: str) -> dict[str, Any] | None:
    ...
    assert job.job_id == job_id, f"Job ID mismatch: expected {job_id}, got {job.job_id}"
    ...
    try:
        ...
        assert prepared.path.exists(), f"Prepared config file must exist: {prepared.path}"
        ...
        if job.config_overrides:
            rollout_budget_override = job.config_overrides.get(
                "prompt_learning.gepa.rollout.budget"
            )
            max_rollouts_override = job.config_overrides.get(
            ...
            if rollout_budget_override is not None:
                ...
                rollout_section = (
                    gepa_section.get("rollout", {}) if isinstance(gepa_section, dict) else {}
                )
                extracted_budget = (
                    rollout_section.get("budget") if isinstance(rollout_section, dict) else None
                )
            ...
            if max_rollouts_override is not None:
                ...
                extracted_max_rollouts = (
                    termination_section.get("max_rollouts")
                    if isinstance(termination_section, dict)
                    else None
                )
        ...
        if final_model or environment:
            ...
            policy_str = (
                f"{final_provider}/{final_model}" if final_provider and final_model else final_model
            )
                ...
                for line in process.stdout:
                    ...
                    if not backend_job_id:
                        ...
                        if extracted_id:
                            ...
                            logger.info(
                                "📋 Stored backend_job_id in status_json for job %s", job.job_id
                            )
                        ...
                        if (
                            ...
                            logger.info(
                                "📡 Started progress poller for backend job %s", backend_job_id
                            )
                        else:
                            logger.warning(
                    ...
                    should_update = (
                        ...
                        "rollouts=" in line.lower()
                        or "progress:" in line.lower()
                        or "gepa progress:" in line.lower()
                        or
                    ...
                    if should_update:
                        ...
                        output_to_parse = accumulated_output[
                            -5000:
                        ]  # Last 5KB to avoid parsing huge outputs
        ...
        if (
            "health check failed" in combined_lower
            or "aborting due to failing health check" in combined_lower
        ):
            ...
            for source_name, source_text in [
                ...
                if "health check" in source_lower:
                    ...
                    health_check_failures.append(
                        f"{source_name} (health check context):\n{source_text[start:end]}"
                    )
                    ...
                    if "500" in source_text or "internal server error" in source_lower:
                        ...
                        error_idx = (
                            source_lower.find("500")
                            if "500" in source_text
                            else source_lower.find("internal server error")
                        )
                        if error_idx >= 0:
                            error_start = max(0, error_idx - 100)
                            error_end = min(len(source_text), error_idx + 800)
                            health_check_details.append(
                                f"{source_name} (500 error details):\n{source_text[error_start:error_end]}"
                            )

                    # Look for tracebacks or exception messages
                    if (
                        "traceback" in source_lower
                        or "exception" in source_lower
                        or "error:" in source_lower
                    ):
                        # Find traceback/exception
                        tb_idx = (
                            source_lower.find("traceback")
                            if "traceback" in source_lower
                            else (
                                source_lower.find("exception")
                                if "exception" in source_lower
                                else source_lower.find("error:")
                            )
                        )
                        if tb_idx >= 0:
                            tb_start = max(0, tb_idx - 50)
                            tb_end = min(
                                len(source_text), tb_idx + 1500
                            )  # Get more context for tracebacks
                            health_check_details.append(
                                f"{source_name} (exception/traceback):\n{source_text[tb_start:tb_end]}"
                            )
                    ...
                    for pattern in error_patterns:
                        ...
                        for match in matches:
                            ...
                            health_check_details.append(
                                f"{source_name} (error pattern '{pattern[:30]}...'):\n{source_text[match_start:match_end]}"
                            )
        ...
        if completed.stdout:
            if not success:
                ...
                logger.error(
                    "Training command stdout (FULL, %d chars):\n%s",
                ...
                logger.info(
                    "Training command stdout (last 2000 chars):\n%s", completed.stdout[-2000:]
                )
        else:
            logger.warning(
                "Training command stdout is EMPTY - command may have exited before producing output"
            )
        ...
        if completed.stderr:
            if not success:
                # For errors, log full output
                logger.error(
                    "Training command stderr (FULL, %d chars):\n%s",
                ...
                # For success, log last 2000 chars
                logger.warning(
                    "Training command stderr (last 2000 chars):\n%s", completed.stderr[-2000:]
                )
        else:
                ...
                if backend_url and api_key:
                    ...
                    if resp.status_code == 200:
                        ...
                        if backend_total_rollouts is None:
                            opt_rollouts = (
                                backend_stats.get("optimization_rollouts_executed", 0) or 0
                            )
...
@celery_app.task(name="synth_ai.core.experiment_queue.process_experiment_queue")
def process_experiment_queue() -> dict[str, Any]:
    ...
    with session_scope() as session:
        ...
        active_experiments = (
            ...
            .filter(Experiment.status.in_([ExperimentStatus.QUEUED, ExperimentStatus.RUNNING]))
            .all()
        )
