...
from synth_ai.core.tunnels.cfgs import CFDeployCfg
...
def get_cloudflared_path(prefer_system: bool = True) -> Optional[Path]:
    ...
    from synth_ai.core.utils.paths import SYNTH_BIN_DIR
    ...
    while True:
        ...
        try:
            ...
            curl_cmd = [
                ...
                "-k",
                "--resolve",
                f"{hostname}:{port_num}:{resolved_ip}",
                test_url,
            ]
            if check_api_key:
            ...
            if status_code == 530:
                ...
                stderr_msg = (
                    result.stderr[:100] if result.stderr else "timeout/connection refused"
                ).strip()
...
def store_tunnel_credentials(
    ...
    try:
        from synth_ai.core.config.user import update_user_config
...
async def deploy_app_tunnel(
    ...
    try:
        if cfg.mode == "quick":
            ...
            try:
                from synth_ai.core.tunnels.service_records import record_tunnel
        ...
        try:
            from synth_ai.core.tunnels.service_records import remove_tunnel_record
