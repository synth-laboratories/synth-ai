...
class TunneledLocalAPI:
    ...
    async def _create_quick(
        ...
        url, proc = await asyncio.to_thread(
            ...
            local_port,
            30.0,  # Increased timeout for tunnel URL
        )
        ...
        if verify_dns:
            try:
                ...
                logger.warning("[TUNNELED_API] DNS verification failed for quick tunnel: %s", e)
    ...
    def close(self) -> None:
        ...
        if self._handle:
            try:
                ...
                lease_id = getattr(self._handle, "lease_id", None)
                if lease_id:
                    synth_ai_py.tunnel_close(lease_id)
            except Exception as e:
