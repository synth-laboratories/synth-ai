...
class CalibrationExample:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            not isinstance(self.outcome_reward, (int, float))
            or self.outcome_reward < 0.0
            or self.outcome_reward > 1.0
        ):
            raise ValueError(
...
class GoldExample:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            not isinstance(self.gold_score, (int, float))
            or self.gold_score < 0.0
            or self.gold_score > 1.0
        ):
            raise ValueError(
        ...
        if (
            not self.gold_reasoning
            or not isinstance(self.gold_reasoning, str)
            or not self.gold_reasoning.strip()
        ):
