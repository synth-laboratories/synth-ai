...
from synth_ai.sdk.api.train.graphgen_models import load_graphgen_taskset
...
_logger.debug("[TRAIN_MODULE] Module synth_ai.cli.train imported")
...
def handle_graphgen(
    *,
    ...
    try:
        ...
        raise click.ClickException(f"Dataset file not found: {dataset_path}") from e
    except ValueError as e:
        raise click.ClickException(f"Invalid GraphGen dataset format: {e}") from e
...
def _save_verbose_log_file(
    ...
    try:
        ...
        try:
            from synth_ai.sdk.api.train.summary import _generate_summary_text
    ...
    try:
        ...
        if mipro_topk_candidates and isinstance(mipro_topk_candidates, list):
            ...
            for cand in mipro_topk_candidates:
                ...
                lines.append(
                    f"[Rank {rank}] Train: {train_score:.4f} ({train_score * 100:.1f}%) | Test: {test_score:.4f} ({test_score * 100:.1f}%){lift_str}"
                )
                lines.append("-" * 80)
                ...
                if stage_payloads:
                    ...
                    for stage_id, payload in stage_payloads.items():
                        if isinstance(payload, dict):
                            ...
                            lines.append(
                                f"  [{module_id}/{stage_id}] instr_ids={instr_ids} demo_ids={demo_ids}"
                            )
        ...
        if proposed_instructions and isinstance(proposed_instructions, list):
            ...
            for idx, instr in enumerate(proposed_instructions):
                ...
                lines.append(
                    f"[{idx + 1}] Iteration {iteration} | Stage: {stage_id} | Module: {module_id} | ID: {instruction_id}"
                )
                if demo_indices:
                    lines.append(f"Demo Indices: {demo_indices}")
                lines.append("-" * 80)
...
def handle_prompt_learning(
    *,
    ...
    if verbose_summary and algorithm in ("gepa", "mipro"):
        ...
        from synth_ai.sdk.api.train.summary import display_prompt_learning_summary
