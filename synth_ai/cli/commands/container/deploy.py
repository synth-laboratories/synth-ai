"""Deploy a Container as a cloud container."""
...
@click.option(
    "--entrypoint",
    "-e",
    help="Entrypoint command to start the Container server.",
)
@click.option(
    "--entrypoint-mode",
...
@click.option(
    ...
    default=8000,
    show_default=True,
    help="Port for the Container server (used with --app).",
)
@click.option(
    "--dockerfile",
    "-d",
...
def deploy(
    ...
    from synth_ai.sdk.container import deploy_container
    from synth_ai.sdk.harbor import HarborLimits

    _echo(f"Deploying Container '{name}'...")
    ...
    try:
        ...
        if not wait and result.status != "ready":
            ...
            if provider == "harbor":
                ...
                click.echo("  synth container deploy --wait ...")
        if result.status == "ready":
            click.echo("\nContainer is ready to accept /rollout traffic.")
