...
def parse_model_id(model_id: str) -> ParsedModelId:
    ...
    if model_id.startswith("peft:"):
        ...
        if len(parts) < 3:
            raise ValueError(
                f"Invalid peft: model ID format. Expected 'peft:BASE_MODEL:JOB_ID', got: {model_id}"
            )
        ...
        if len(parts) < 3:
            raise ValueError(
                f"Invalid ft: model ID format. Expected 'ft:BASE_MODEL:JOB_ID', got: {model_id}"
            )
        return ParsedModelId(
            prefix=prefix,
            base_model=parts[1],
    ...
    # Check for RL model prefix
        ...
        if len(parts) < 3:
            raise ValueError(
                f"Invalid rl: model ID format. Expected 'rl:BASE_MODEL:JOB_ID', got: {model_id}"
            )
        return ParsedModelId(
            prefix=prefix,
            base_model=parts[1],
