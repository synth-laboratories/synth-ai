...
SourceType = Literal["web", "local"]


@click.command()
@click.option(
    "--source",
    type=click.Choice(list(typing.get_args(SourceType)), case_sensitive=False),
    default="web",
    show_default=True,
...
def setup_cmd(source: SourceType = "web", approve: bool = False) -> None:
    credentials = {}
    match source:
        case "local":
        ...
        case "web":
            if not approve:
                approve = click.confirm(
                    "This will open your web browser for authentication. Continue?", default=True
                )
                if not approve:
                    return
