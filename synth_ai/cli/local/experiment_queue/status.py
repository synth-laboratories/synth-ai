...
class ExperimentStatus:
    ...
    def __post_init__(self) -> None:
        ...
        if self.eta_seconds is not None:
            assert self.eta_seconds >= 0, f"eta_seconds must be >= 0, got {self.eta_seconds}"
        ...
        if self.current_trial is not None:
            assert self.current_trial >= 0, f"current_trial must be >= 0, got {self.current_trial}"
    ...
    def from_dict(cls, data: dict[str, Any]) -> ExperimentStatus:
        ...
        """
        assert isinstance(data, dict), f"data must be dict, got {type(data).__name__}: {data}"
        ...
        def coerce_int(key: str, value: Any) -> int | None:
            ...
            raise AssertionError(f"{key} must be int | None, got {type(value).__name__}: {value}")
        ...
        def coerce_float(key: str, value: Any) -> float | None:
            ...
            raise AssertionError(f"{key} must be float | None, got {type(value).__name__}: {value}")
