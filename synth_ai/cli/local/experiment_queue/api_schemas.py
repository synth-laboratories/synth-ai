...
class ProgressEventData(BaseModel):
    ...
    rollouts_remaining: int | None = Field(
        None, description="Rollouts remaining (may be negative due to timing/calculation issues)"
    )
    rollout_budget_display: int | None = Field(
        None, gt=0, description="Alternative field name for rollouts_total"
    )
    percent_rollouts: float | None = Field(
        None, ge=0.0, le=1.0, description="Rollout progress (0-1)"
    )
    percent_overall: float | None = Field(
        None, ge=0.0, le=1.0, description="Overall progress (0-1)"
    )
    transformations_planned: int | None = Field(
        None, ge=0, description="Total transformations planned"
    )
    transformations_tried: int | None = Field(None, ge=0, description="Transformations tried")
    percent_transformations: float | None = Field(
        None, ge=0.0, le=1.0, description="Transformation progress (0-1)"
    )
    elapsed_seconds: int | None = Field(None, ge=0, description="Elapsed time in seconds")
    eta_seconds: float | None = Field(
        None, ge=0.0, description="Estimated time remaining in seconds"
    )
    ...
    accuracy: float | None = Field(
        None, ge=0.0, le=1.0, description="Alternative field name for best_score"
    )
