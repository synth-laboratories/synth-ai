...
def plot_optimization_curve(
    ...
    *,
    ...
    for i in range(len(trial_counts) - 1):
        x1 = (
            int((trial_counts[i] - min_trial) / (max_trial - min_trial) * (width - 1))
            if max_trial != min_trial
            else width - 1
        )
        y1 = (
            int((max_score - best_scores[i]) / score_range * (height - 1))
            if score_range != 0
            else height - 1
        )
        x2 = (
            int((trial_counts[i + 1] - min_trial) / (max_trial - min_trial) * (width - 1))
            if max_trial != min_trial
            else width - 1
        )
        y2 = (
            int((max_score - best_scores[i + 1]) / score_range * (height - 1))
            if score_range != 0
            else height - 1
        )
