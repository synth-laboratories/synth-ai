...
def handle_prompt_learning(
    *,
    ...
    # Augment final_status with top-K prompts and their train/val scores
    try:
        ...
        # First, collect validation scores by rank
        for event in events:
            if event.get("type") == "prompt.learning.validation.scored":
                msg = event.get("message", "")
                data = event.get("data", {})
                # Parse "top[0] val_accuracy=0.620 (N=50) lift=0.0"
                if "top[" in msg:
                    try:
                        rank_str = msg.split("top[")[1].split("]")[0]
                        rank = int(rank_str)
                        val_acc = data.get("accuracy") or float(msg.split("val_accuracy=")[1].split()[0])
                        validation_scores[rank] = val_acc
                    except (IndexError, ValueError, AttributeError):
                        pass
        
        # Then, collect optimized candidates
        for event in events:
            if event.get("type") == "prompt.learning.optimized.scored":
                msg = event.get("message", "")
                data = event.get("data", {})
                # Parse "optimized[0] train_accuracy=0.2 len=86 N=5 val_accuracy=0.120"
                try:
                    ...
                    # Extract validation accuracy if present in message or from validation_scores
                    val_acc = None
                    if "val_accuracy=" in msg:
                        val_acc = float(msg.split("val_accuracy=")[1].split()[0])
                    elif idx in validation_scores:
                        val_acc = validation_scores[idx]
                    
                    # Extract prompt preview
                    prompt_preview = ""
                    if "TRANSFORMATION:" in msg or "✨" in msg:
        ...
        if top_k_results:
            final_status["top_k_prompts"] = top_k_results
            click.echo(f"\n✨ Top-{len(top_k_results)} Prompts:")
            for result in top_k_results:
                val_str = f"{result['val_accuracy']:.3f}" if result['val_accuracy'] is not None else "N/A"
                click.echo(f"  [{result['rank']}] train={result['train_accuracy']:.3f} val={val_str}")
    except Exception as e:
        click.echo(f"⚠️  Could not extract top-K results: {e}", err=True)
    
    click.echo(preview_json(final_status, limit=800))
