...
var maxRefreshInterval = parseFloat(process.env.SYNTH_TUI_REFRESH_MAX || "60");
var maxEventInterval = parseFloat(process.env.SYNTH_TUI_EVENT_MAX || "15");
var eventHistoryLimit = parseInt(process.env.SYNTH_TUI_EVENT_CARDS || "200", 10);
...
var jobsPollMs = Math.max(1, refreshInterval) * 1000;
var eventsPollMs = Math.max(0.5, eventInterval) * 1000;
var jobsInFlight = false;
...
var promptBrowserIndex = 0;
var promptBrowserOffset = 0;
...
renderer.keyInput.on("keypress", (key) => {
  ...
  if (key.name === "tab")
    setActivePane(activePane === "jobs" ? "events" : "jobs");
  if (key.name === "b")
    setActivePane("jobs");
  if (key.name === "r")
    refreshJobs();
...
async function bootstrap() {
  ...
  setInterval(refreshIdentity, 60000);
  renderSnapshot();
}
...
async function selectJob(jobId) {
  ...
  snapshot.selectedJob = immediate ?? {
    job_id: jobId,
    status: "loading",
    training_type: null,
    created_at: null,
...
async function fetchAllCandidates(token) {
  ...
  const relevantEvents = snapshot.events.filter((ev) => {
    const t2 = ev.type || "";
    return t2.includes("candidate.evaluated") || t2.includes("proposal.scored") || t2.includes("optimized.scored");
  });
  const seenVersionIds = new Set(["baseline", "baseline_transformation"]);
  for (const ev of relevantEvents) {
    const payload = ev.payload || ev.data || {};
    const versionId = payload.version_id || payload.program_candidate?.candidate_id;
    if (!versionId || seenVersionIds.has(versionId))
      continue;
  ...
  try {
    ...
    if (Array.isArray(artifacts)) {
      for (const art of artifacts) {
        ...
        if (!snapId || seenVersionIds.has(snapId))
          continue;
        try {
  ...
  try {
    ...
    if (events.length > 0) {
      ...
      if (eventHistoryLimit > 0 && snapshot.events.length > eventHistoryLimit) {
        ...
        eventWindowStart = clamp(eventWindowStart, 0, Math.max(0, snapshot.events.length - Math.max(1, eventVisibleCount)));
      }
...
function renderSnapshot() {
  ...
  ui.jobsBox.title = jobStatusFilter.size ? `Jobs (status: ${Array.from(jobStatusFilter).join(", ")})` : "Jobs";
  ui.jobsSelect.options = filteredJobs.length ? filteredJobs.map((job) => {
    ...
    {
      name: "no jobs",
      description: jobStatusFilter.size ? `no jobs with selected status` : "no prompt-learning jobs found",
      value: ""
    }
  ];
...
function getFilteredJobs() {
  ...
  return snapshot.jobs.filter((job) => jobStatusFilter.has(String(job.status || "unknown").toLowerCase()));
}
...
function collectJobStatuses(jobs) {
...
function extractBestPromptText(snapshotPayload) {
  if (!snapshotPayload)
    return null;
  const text = snapshotPayload.best_prompt_text || snapshotPayload.best_prompt_preview || snapshotPayload.best_prompt_rendered;
  return typeof text === "string" ? text : null;
}
function extractPromptSections(bestPrompt) {
  if (!bestPrompt)
    return [];
  if (Array.isArray(bestPrompt.sections))
    return bestPrompt.sections;
  if (Array.isArray(bestPrompt.prompt_sections))
    return bestPrompt.prompt_sections;
  return [];
}
...
function toggleFilterModal(visible) {
...
function toggleJobFilterModal(visible) {
...
function toggleSelectedJobFilter() {
...
function moveJobFilterSelection(delta) {
...
function applyJobFilterSelection() {
  const filteredJobs = getFilteredJobs();
  if (!filteredJobs.length) {
    ...
    selectedEventIndex = 0;
    eventWindowStart = 0;
    snapshot.status = jobStatusFilter.size ? "No jobs with selected status" : "No prompt-learning jobs found";
    renderSnapshot();
    return;
  }
...
function togglePromptBrowserModal(visible) {
...
async function openPromptBrowserModal() {
  const job = snapshot.selectedJob;
  if (!job || job.job_source === "learning" || isEvalJob(job)) {
...
function movePromptBrowserScroll(delta) {
...
function movePromptBrowserCandidate(delta) {
...
function updatePromptBrowserContent() {
  if (!ui.promptBrowserVisible)
    return;
...
function formatCandidateContent(candidate) {
  if (!candidate)
  ...
  if (candidate.isBaseline) {
    ...
    if (payload2) {
      if (payload2.generation != null)
        lines.push(`Generation: ${payload2.generation}`);
      if (payload2.mutation_type)
        lines.push(`Mutation: ${payload2.mutation_type}`);
      if (payload2.status)
        lines.push(`Status: ${payload2.status}`);
    }
  }
  lines.push("");
...
`);
  ...
  if (promptText && typeof promptText === "string" && promptText.length > 0) {
    ...
    lines.push("");
    const stages2 = payload.stages;
    if (stages2 && typeof stages2 === "object" && Object.keys(stages2).length > 0) {
      lines.push("");
      lines.push("=== STAGES (STRUCTURED) ===");
      for (const [stageId, stageData] of Object.entries(stages2)) {
...
`);
      ...
      if (promptConfig.name || promptConfig.template) {
        lines.push("=== INITIAL PROMPT ===");
        lines.push("");
        if (promptConfig.name)
          lines.push(`Name: ${promptConfig.name}`);
...
function getPromptBrowserClipboardContent() {
  const candidates = snapshot.allCandidates;
  if (candidates.length === 0)
    return null;
  const candidate = candidates[promptBrowserIndex];
...
function moveEventModal(delta) {
  if (!ui.eventModalPayload)
    return;
...
function updateEventModalContent() {
  if (!ui.eventModalVisible || !ui.eventModalPayload)
    return;
...
function moveEventSelection(delta) {
...
function scheduleEventsPoll(delayMs) {
  if (eventsTimer)
...
function buildLayout(renderer2) {
  ...
  renderer2.root.add(root);
  const headerBox = new BoxRenderable(renderer2, {
    id: "header-box",
    width: "auto",
    height: 3,
  ...
  const headerText = new TextRenderable(renderer2, {
    id: "header-text",
    content: "Synth AI Prompt Learning Monitor",
    fg: "#e2e8f0"
  });
  const headerSpacer = new BoxRenderable(renderer2, {
    id: "header-spacer",
    width: "auto",
    height: "auto",
    flexGrow: 1,
  ...
  const headerMetaText = new TextRenderable(renderer2, {
    id: "header-meta-text",
    content: "",
    fg: "#94a3b8"
  });
  ...
  root.add(headerBox);
  const tabsBox = new BoxRenderable(renderer2, {
    id: "tabs-box",
    width: "auto",
    height: 2,
  ...
  const jobsTabText = new TextRenderable(renderer2, {
    id: "tabs-jobs",
    content: "[Jobs] (b)",
    fg: "#f8fafc"
  });
  const eventsTabText = new TextRenderable(renderer2, {
    id: "tabs-events",
    content: "[Events] (e)",
    fg: "#94a3b8"
  });
  ...
  root.add(tabsBox);
  const main2 = new BoxRenderable(renderer2, {
    id: "main",
    width: "auto",
    height: "auto",
    flexDirection: "row",
  ...
  root.add(main2);
  const jobsBox = new BoxRenderable(renderer2, {
    id: "jobs-box",
    width: 36,
    height: "auto",
    minWidth: 36,
  ...
  const jobsSelect = new SelectRenderable(renderer2, {
    id: "jobs-select",
    width: "auto",
    height: "auto",
    options: [],
  ...
  main2.add(jobsBox);
  const detailColumn = new BoxRenderable(renderer2, {
    id: "detail-column",
    width: "auto",
    height: "auto",
    flexDirection: "column",
  ...
  const detailBox = new BoxRenderable(renderer2, {
    id: "detail-box",
    width: "auto",
    height: 12,
    borderStyle: "single",
  ...
  const detailText = new TextRenderable(renderer2, {
    id: "detail-text",
  ...
  const resultsBox = new BoxRenderable(renderer2, {
    id: "results-box",
    width: "auto",
    height: 6,
    borderStyle: "single",
  ...
  const resultsText = new TextRenderable(renderer2, {
    id: "results-text",
    content: "Results: -",
    fg: "#e2e8f0"
  });
  ...
  const metricsBox = new BoxRenderable(renderer2, {
    id: "metrics-box",
    width: "auto",
    height: 5,
    borderStyle: "single",
  ...
  const metricsText = new TextRenderable(renderer2, {
    id: "metrics-text",
    content: "Metrics: -",
    fg: "#cbd5f5"
  });
  ...
  const eventsBox = new BoxRenderable(renderer2, {
    id: "events-box",
    width: "auto",
    height: "auto",
    flexGrow: 1,
  ...
  const eventsList = new BoxRenderable(renderer2, {
    id: "events-list",
    width: "auto",
    height: "auto",
    flexDirection: "column",
  ...
  const eventsEmptyText = new TextRenderable(renderer2, {
  ...
  const statusBox = new BoxRenderable(renderer2, {
    id: "status-box",
    width: "auto",
    height: 3,
    backgroundColor: "#0f172a",
  ...
  const statusText = new TextRenderable(renderer2, {
    id: "status-text",
    content: "Ready.",
    fg: "#e2e8f0"
  });
  ...
  root.add(statusBox);
  const footerBox = new BoxRenderable(renderer2, {
    id: "footer-box",
    width: "auto",
    height: 2,
    backgroundColor: "#111827",
  ...
  const footerTextNode = new TextRenderable(renderer2, {
    id: "footer-text",
    content: footerText(),
    fg: "#94a3b8"
  });
  ...
  root.add(footerBox);
  const modalBox = new BoxRenderable(renderer2, {
    id: "modal-box",
    width: 50,
    height: 5,
    position: "absolute",
  ...
  const modalLabel = new TextRenderable(renderer2, {
    id: "modal-label",
    content: "Snapshot ID:",
    fg: "#e2e8f0",
    position: "absolute",
  ...
  renderer2.root.add(modalBox);
  renderer2.root.add(modalLabel);
  renderer2.root.add(modalInput);
  const filterBox = new BoxRenderable(renderer2, {
    id: "filter-box",
    width: 52,
    height: 5,
    position: "absolute",
  ...
  const filterLabel = new TextRenderable(renderer2, {
    id: "filter-label",
  ...
  renderer2.root.add(filterBox);
  renderer2.root.add(filterLabel);
  renderer2.root.add(filterInput);
  const jobFilterBox = new BoxRenderable(renderer2, {
    id: "job-filter-box",
    width: 52,
    height: 11,
    position: "absolute",
  ...
  const jobFilterLabel = new TextRenderable(renderer2, {
    id: "job-filter-label",
  ...
  const jobFilterHelp = new TextRenderable(renderer2, {
    id: "job-filter-help",
  ...
  const jobFilterListText = new TextRenderable(renderer2, {
    id: "job-filter-list",
    content: "",
    fg: "#e2e8f0",
    position: "absolute",
  ...
  renderer2.root.add(jobFilterBox);
  renderer2.root.add(jobFilterLabel);
  renderer2.root.add(jobFilterHelp);
  renderer2.root.add(jobFilterListText);
  const eventModalBox = new BoxRenderable(renderer2, {
    id: "event-modal-box",
    width: 80,
    height: 16,
    position: "absolute",
  ...
  const eventModalTitle = new TextRenderable(renderer2, {
  ...
  const eventModalText = new TextRenderable(renderer2, {
    id: "event-modal-text",
    content: "",
    fg: "#e2e8f0",
    position: "absolute",
  ...
  const eventModalHint = new TextRenderable(renderer2, {
    id: "event-modal-hint",
    content: "Event details",
    fg: "#94a3b8",
    position: "absolute",
  ...
  renderer2.root.add(eventModalBox);
  ...
  const promptBrowserBox = new BoxRenderable(renderer2, {
  ...
  const promptBrowserTitle = new TextRenderable(renderer2, {
  ...
  const promptBrowserText = new TextRenderable(renderer2, {
  ...
  const promptBrowserHint = new TextRenderable(renderer2, {
  ...
  renderer2.root.add(promptBrowserBox);
  renderer2.root.add(promptBrowserTitle);
  renderer2.root.add(promptBrowserText);
  renderer2.root.add(promptBrowserHint);
