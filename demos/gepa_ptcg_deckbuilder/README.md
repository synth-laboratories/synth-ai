...
```bash
uv run python generate_and_score.py --challenge basic-deck --n 1 --games-per-opponent 10
```

Full default scoring is **all 6 comparison decks** at **500 games each**:

```bash
uv run python generate_and_score.py --challenge basic-deck --n 1
```

If you want to score only against the challenge’s configured opponent set:

```bash
uv run python generate_and_score.py --challenge basic-deck --use-challenge-opponents
```
