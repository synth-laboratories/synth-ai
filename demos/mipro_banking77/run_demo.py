...
def register_candidate(
    ...
    candidate_prompt = f"{baseline_prompt} Always call the tool and never answer directly."
    transformation = {
        "text_replacements": [
            {
                "old_text": baseline_prompt,
                "new_text": candidate_prompt,
    ...
    deltas = {"transformation": transformation}
    headers = {"Authorization": f"Bearer {api_key}"}
    response = httpx.post(
        f"{backend_url}/api/prompt-learning/online/mipro/systems/{system_id}/candidates",
        json={"deltas": deltas, "metadata": {"label": "strict_tool_call"}},
        headers=headers,
        timeout=30.0,
    )
    response.raise_for_status()
    ...
    if not candidate_id:
        raise RuntimeError(f"Candidate registration failed: {payload}")
...
def main() -> None:
    ...
    if args.mode in {"online", "both"}:
        ...
        candidate_id = register_candidate(
            ...
            api_key,
            system_id,
            localapi_banking77.SYSTEM_PROMPT,
        )
        ...
        for seed in seeds:
            ...
            print(f"Rollout {seed}: reward={reward:.3f} id={rollout_id}")
            push_status(backend_url, api_key, system_id, rollout_id, reward)
