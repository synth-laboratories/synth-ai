#!/usr/bin/env python3
"""Run RLM v1 verifier on OOLONG-style large context benchmark.

This demo tests the RLM v1 verifier's ability to handle large context
retrieval tasks by:
1. Creating OOLONG-style contexts with hidden answers in large documents
2. Running RLM v1 verifier to find and extract information
3. Comparing results against ground truth

Usage:
    uv run python demos/rlm/run_rlm_v1_oolong.py           # Production mode
    uv run python demos/rlm/run_rlm_v1_oolong.py --local   # Local mode
    uv run python demos/rlm/run_rlm_v1_oolong.py --local --context-size 50000
"""
...
parser = argparse.ArgumentParser(description="Run RLM v1 OOLONG benchmark")
parser.add_argument(
    "--local",
    action="store_true",
...
VERIFIER_GRAPH_ID = "zero_shot_verifier_rubric_rlm"  # RLM v1
...
async def run_rlm_v1_on_question(
    ...
    try:
        ...
        # Extract reward from various possible locations
...
async def main():
    ...
    print(f"\nRunning RLM v1 verifier on {len(questions)} questions...")
    print("-" * 70)
    ...
    for i, q in enumerate(questions, 1):
        ...
        result = await run_rlm_v1_on_question(client, context, q["question"], rubric)
        result["gold_answer"] = q["gold_answer"]
        results.append(result)

        status = "OK" if result["reward"] is not None else f"ERROR: {result['error'][:50]}"
