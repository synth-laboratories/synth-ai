...
async def task_info() -> dict[str, Any]:
    return {
        "taskset": {
            "name": "style_matching",
            "description": "Style matching demo localapi",
            "size": len(TASKS),
        }
    }
...
async def _score_with_verifier(
    ...
    payload = {
        ...
        "input": {
            ...
            "candidate_score": 0.5,
            "candidate_reasoning": "Auto-evaluated from style-matching localapi",
...
async def run_gepa_with_verifier(label: str, verifier_job_id: str) -> tuple[str, Any]:
    ...
    config_body = {
        "prompt_learning": {
            ...
            "verifier": {"enabled": False, "reward_source": "localapi"},
        }
    }
...
async def main() -> None:
    ...
    print(f"Starting localapi on port {LOCAL_TASK_PORT}...")
    with _localapi_lock:
        _start_localapi()
    print("Localapi ready!")
    _start_localapi_monitor()

    if USE_LOCAL_BACKEND:
        print("Using localhost localapi URL (no tunnel)")
