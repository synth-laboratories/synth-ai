...
from synth_ai.cli.status import session as session_command
from synth_ai.sdk.session.models import AgentSession, AgentSessionLimit, AgentSessionUsage
...
def test_session_status_with_session_id(runner: CliRunner, mock_session):
    ...
    with (
        mock.patch("synth_ai.cli.status.resolve_backend_config") as mock_config,
        mock.patch("synth_ai.cli.status.AgentSessionClient") as mock_client_class,
    ):
        mock_cfg = mock.Mock()
        ...
        result = runner.invoke(session_command, ["--session-id", "sess_test123"])
...
def test_session_status_from_env(runner: CliRunner, mock_session, monkeypatch):
    ...
    with (
        ...
        result = runner.invoke(session_command)
...
def test_session_status_with_exceeded_limit(runner: CliRunner):
    ...
    with (
        ...
        result = runner.invoke(session_command, ["--session-id", session_id])
...
def test_session_status_session_not_found(runner: CliRunner):
    ...
    with (
        ...
        result = runner.invoke(session_command, ["--session-id", "sess_notfound"])
