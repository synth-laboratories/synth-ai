...
from synth_ai.core.experiment_queue import celery_app, config
...
def test_database_path_required():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert cfg.sqlite_path == db_path.resolve(), (
            f"Expected {db_path.resolve()}, got {cfg.sqlite_path}"
        )
...
def test_database_path_consistency():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert app.conf.broker_url.startswith("redis://"), (
            f"Expected Redis broker, got: {app.conf.broker_url}"
        )
...
def test_celery_app_recreation_on_config_change():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        config.load_config()  # Reload to get current (db2)
...
def test_single_database_enforcement():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert app2.conf.broker_url.startswith("redis://"), (
            f"Expected Redis broker, got: {app2.conf.broker_url}"
        )
