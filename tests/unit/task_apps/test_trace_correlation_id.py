...
def test_extract_trace_correlation_id_from_modal_url():
    ...
    extractor = utils_mod.extract_trace_correlation_id
    url = "https://synth-labs--crafter.modal.run/v1/chat/completions?cid=trace_run-1234abcd&foo=bar"
...
def test_resolve_trace_correlation_id_uses_inference_url(mode):
    ...
    resolver = grpo_mod._resolve_trace_correlation_id
    policy_cfg = {
        "model": "Qwen/Qwen3-4B",
        "inference_url": (
            "https://ta-01k8skyc1cgtrtep1avyyqhs3s-8000.modal.host?cid=trace_run-e2f1b3da"
        ),
    }
...
class TestExtractTraceCorrelationIdPathBased:
    ...
    def test_extract_from_query_param_fallback(self):
        ...
        policy_cfg = {}
        inference_url = (
            "http://localhost:8115/v1/baseline-0-abc123/chat/completions?cid=trace_query_only"
        )
        result = extract_trace_correlation_id(policy_cfg, inference_url, mode="rl")
    ...
    def test_policy_config_takes_precedence(self):
        ...
        inference_url = (
            "http://localhost:8115/v1/baseline-0-abc123/trace_from_path/chat/completions"
        )
        result = extract_trace_correlation_id(policy_cfg, inference_url, mode="rl")
