...
def test_required_mode_denies_without_environment_key(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("SYNTH_CONTAINER_AUTH_MODE", "required")
    monkeypatch.delenv("ENVIRONMENT_API_KEY", raising=False)
    monkeypatch.setattr(auth, "synth_ai_py", None)
...
def test_required_mode_ignores_environment_key_headers(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("SYNTH_CONTAINER_AUTH_MODE", "required")
    ...
    with pytest.raises(Exception) as err:
        auth.require_api_key_dependency(
            _Request(path="/rollout", headers={"x-api-key": "env-key"}, host="10.0.0.7")
        )
    _assert_http_error(err.value, 401, "auth_missing")
...
def test_paseto_records_adoption_metric_without_env_key_warning(
    ...
    warning_messages = [r.getMessage().lower() for r in caplog.records if r.levelname == "WARNING"]
    assert all("env-key auth fallback is deprecated" not in message for message in warning_messages)
