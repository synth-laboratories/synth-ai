#!/usr/bin/env python3
"""Integration test script for GEPA with proxy models and adaptive pooling.

This script emulates the monorepo test_gepa_banking77.py but uses the synth-ai SDK
to build and submit jobs to the local backend.

Run with:
    REDIS_URL=redis://127.0.0.1:6379/0 \
    uv run python -m tests.integration.prompt_learning.test_gepa_proxy_adaptive_local \
      --config path/to/banking77_gepa_proxy_adaptive.toml
...
def main():
    parser = argparse.ArgumentParser(description="Run GEPA optimization with proxy models and adaptive pooling")
    parser.add_argument(
        "--config",
        type=str,
        default=None,
        help="Path to TOML config file (defaults to monorepo banking77_gepa_proxy_adaptive.toml)",
    )
    parser.add_argument(
        "--backend",
        type=str,
    ...
    if args.config:
        ...
        if MONOREPO_BACKEND and MONOREPO_BACKEND.exists():
            config_path = (
                ...
                / "prompt_learning"
                / "configs"
                / "banking77_gepa_proxy_adaptive.toml"
            )
        else:
    ...
    print("=" * 80)
    print("GEPA Test Run: Banking77 (Proxy Models + Adaptive Pooling)")
    print("=" * 80)
    ...
    # Verify config includes proxy models and adaptive pooling
    ...
    proxy_models = pl_config.get("proxy_models") or gepa_config.get("proxy_models")
    adaptive_pool = gepa_config.get("adaptive_pool")
    adaptive_batch = gepa_config.get("adaptive_batch")
    ...
    if adaptive_batch:
        level = adaptive_batch.get("level", "UNKNOWN")
        print(f"   ✅ Adaptive batch: {level}")
    else:
        print("   ⚠️  Adaptive batch: Not configured")
    print()
        ...
        if args.poll:
            ...
            if results:
                ...
                if isinstance(results, dict):
                    ...
                    # Adaptive batch stats
                    adaptive_batch_stats = results.get("adaptive_batch_stats")
                    if adaptive_batch_stats:
                        net_gain = adaptive_batch_stats.get("net_gain_usd")
                        if net_gain is not None:
                            metrics_rows.append(("Adaptive Batch Net Gain", f"${net_gain:.4f}"))
