...
class TestSqldDaemonBinaryDetection:
    ...
    def test_finds_sqld_in_common_location(self, temp_db_path, mock_sqld_binary, monkeypatch):
        ...
        http_port = 18089
        with patch(
            "synth_ai.core.tracing_v3.sqld.find_sqld_binary", side_effect=mock_find_sqld_binary
        ):
            daemon = SqldDaemon(
                db_path=str(temp_db_path), hrana_port=hrana_port, http_port=http_port
            )
...
class TestSqldDaemonAutoInstall:
    ...
    def test_auto_install_in_interactive_mode(self, temp_db_path, mock_sqld_binary, monkeypatch):
        ...
        with (
            ...
            patch("synth_ai.core.tracing_v3.sqld.install_sqld", side_effect=mock_install_sqld),
            patch("click.confirm", side_effect=mock_confirm),
        ):
            daemon = SqldDaemon(
                db_path=str(temp_db_path), hrana_port=hrana_port, http_port=http_port
            )
    ...
    def test_auto_install_respects_user_decline(self, temp_db_path, monkeypatch):
        ...
        with (
            ...
            patch("click.confirm", side_effect=mock_confirm),
            pytest.raises(RuntimeError, match="sqld binary not found"),
        ):
...
class TestSqldDaemonIntegration:
    ...
    def test_start_sqld_helper_with_auto_install(self, temp_db_path, mock_sqld_binary, monkeypatch):
        ...
        monkeypatch.setattr(
            daemon_module.shutil, "which", lambda cmd: mock_sqld_binary if cmd == "sqld" else None
        )
