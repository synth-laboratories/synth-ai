...
const TRACE_DB_BASENAME: &str = "turso_container_traces";
...
pub fn tracing_env_enabled(default: bool) -> bool {
    let raw = env::var("CONTAINER_TRACING_ENABLED").ok();
    let raw = match raw {
...
pub fn resolve_tracing_db_url() -> Option<String> {
    ...
    if let Some(path_str) = candidate.to_str() {
        env::set_var("CONTAINER_TRACE_DB_PATH", path_str);
...
pub fn resolve_sft_output_dir() -> Option<String> {
    let raw = env::var("CONTAINER_SFT_OUTPUT_DIR")
