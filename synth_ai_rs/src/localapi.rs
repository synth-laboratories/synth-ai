...
pub type RolloutHandler =
    Arc<dyn Fn(RolloutRequest) -> BoxFuture<'static, std::result::Result<RolloutResponse, LocalApiError>>
        + Send
        + Sync>;
...
impl LocalApiConfig {
    ...
    where
        ...
        Fut: Future<Output = std::result::Result<RolloutResponse, LocalApiError>> + Send + 'static,
    {
...
#[derive(Clone)]
pub struct LocalApiApp {
    router: Router,
}

pub fn create_local_api(config: LocalApiConfig) -> LocalApiApp {
...
async fn health(
    State(config): State<Arc<LocalApiConfig>>,
    headers: HeaderMap,
) -> Response {
...
fn authorize(config: &LocalApiConfig, headers: &HeaderMap) -> std::result::Result<(), axum::response::Response> {
...
fn api_key_set(config: &LocalApiConfig) -> HashSet<String> {
