...
def init_dataset(
    ...
    *,
    ...
    # Keep defaults small so demos/CI are fast, but allow callers to scale up.
    train_size = max(1, int(train_size))
    val_size = max(1, int(val_size))
    test_size = max(1, int(test_size))

    trainset = train_items[:train_size]
    valset = train_items[train_size : train_size + val_size]
    test_items = [_format_item(item) for item in dataset["test"]]
    testset = test_items[:test_size]
