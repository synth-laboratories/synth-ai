#!/usr/bin/env python3
"""
Complete MIPRO Prompt Optimization Example
===========================================

This script demonstrates the full pipeline for optimizing prompts using MIPRO:

1. Evaluate initial/baseline prompts
2. Start in-process task app (with Cloudflare tunnel in prod, local in dev)
3. Run MIPRO optimization with programmatic polling
...
async def run_mipro_optimization(
    task_app_url: str,
    backend_url: str,
    api_key: str,
    ...
    """Run MIPRO optimization and return job ID.
    ...
    """
    print(f"\n{'='*80}")
    print("Step 2: Running MIPRO Optimization")
    print(f"{'='*80}\n")

    # Create a temporary config for MIPRO
    import tempfile
    import toml
    ...
    config = {
        "prompt_learning": {
            ...
            "mipro": {
                ...
                "batch_size": 8,
                "bootstrap_train_seeds": list(range(20)),
                "online_pool": list(range(20, 50)),
                "test_pool": list(range(50, 100)),
...
async def evaluate_final(
    ...
    async with httpx.AsyncClient(timeout=300.0) as client:
        ...
        for seed in seeds:
            ...
            # For MIPRO, trial_id is typically the job_id
...
async def main():
    ...
    try:
        ...
        async with InProcessTaskApp(
            ...
            # Step 2: Run MIPRO optimization
            job_id = await run_mipro_optimization(
                task_app_url=task_app.url,
                backend_url=backend_url,
                api_key=api_key,
                task_app_api_key=task_app_api_key,
