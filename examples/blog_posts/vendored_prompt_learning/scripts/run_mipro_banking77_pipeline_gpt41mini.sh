#!/bin/bash
# Run MIPROv2 optimisation for the multi-step Banking77 pipeline using gpt-4.1-mini as policy model

set -euo pipefail

echo "🔬 Running MIPROv2 on Banking77 Pipeline (Policy: gpt-4.1-mini)"
echo "================================================================="
...
if [ -z "${OPENAI_API_KEY:-}" ]; then
    echo "⚠️  Warning: OPENAI_API_KEY not set (required for meta-model gpt-4.1-mini)."
...
CONFIG_PATH="examples/blog_posts/vendored_prompt_learning/configs/banking77_pipeline_mipro_gpt41mini_local.toml"
...
echo "🚀 Starting MIPROv2 training with gpt-4.1-mini as policy model..."
...
echo "  Policy model: gpt-4.1-mini (OpenAI)"
