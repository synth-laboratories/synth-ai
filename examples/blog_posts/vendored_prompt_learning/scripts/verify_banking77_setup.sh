...
if curl -s -f -H "X-API-Key: ${ENVIRONMENT_API_KEY:-dummy}" http://127.0.0.1:8102/health > /dev/null 2>&1; then
    ...
    echo "     Start it with: ./examples/blog_posts/vendored_prompt_learning/scripts/deploy_banking77_task_app.sh"
    MISSING_VARS=$((MISSING_VARS + 1))
fi
...
if curl -s -f -H "X-API-Key: ${ENVIRONMENT_API_KEY:-dummy}" http://127.0.0.1:8112/health > /dev/null 2>&1; then
    ...
    echo "     Start it with: ./examples/blog_posts/vendored_prompt_learning/scripts/deploy_banking77_pipeline_task_app.sh"
    MISSING_VARS=$((MISSING_VARS + 1))
fi
echo ""
...
CONFIG_DIR="examples/blog_posts/vendored_prompt_learning/configs"
...
if [ $MISSING_VARS -eq 0 ]; then
    ...
    echo ""
    echo "Next step:"
    echo "  ./examples/blog_posts/vendored_prompt_learning/scripts/run_mipro_banking77.sh"
    exit 0
else
