#!/usr/bin/env python3
"""Run a short MIPRO optimization for Verilog task app to verify rollouts work."""
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Run short MIPRO test for Verilog")
    parser.add_argument("--task-app-url", default="http://127.0.0.1:8118")
    parser.add_argument("--backend-url", default="http://localhost:8000")
    parser.add_argument("--rollout-budget", type=int, default=5)
    args = parser.parse_args()
    
    config_path = Path(__file__).parent / "verilog_mipro.toml"
    
    print("=" * 80)
    print("🚀 Short MIPRO Test: Verilog")
    print("=" * 80)
    ...
    # Create job with rollout budget override
    ...
    job = PromptLearningJob.from_config(
        ...
        overrides={
            "overrides": {
                "run_local": True,
                "prompt_learning.mipro.rollout_budget": args.rollout_budget,
            }
        },
    )
    ...
    # Poll briefly
    print("Polling for completion (timeout: 120s)...")
    try:
