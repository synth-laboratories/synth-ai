#!/usr/bin/env python3
"""Run GEPA locally on Banking77 via backend endpoint (localhost:8000).
...
Usage:
    python run_gepa_local.py --task-app-url http://127.0.0.1:8102 --rollout-budget 20
"""
...
def create_banking77_gepa_toml(
    task_app_url: str,
    rollout_budget: int = 20,
    train_seeds: Optional[list[int]] = None,
    val_seeds: Optional[list[int]] = None,
) -> str:
    """Create TOML config for Banking77 GEPA."""
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Run GEPA locally on Banking77 via backend endpoint")
    parser.add_argument(
        "--task-app-url",
        type=str,
        default="http://127.0.0.1:8102",
        help="Task app URL (default: http://127.0.0.1:8102)",
    )
    parser.add_argument(
        "--rollout-budget",
        type=int,
    ...
    print("=" * 80)
    print("GEPA Local Test: Banking77 (via Backend Endpoint)")
    print("=" * 80)
    ...
    toml_content = create_banking77_gepa_toml(
