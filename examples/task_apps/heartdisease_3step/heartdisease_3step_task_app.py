...
async def rollout_executor(request: RolloutRequest, fastapi_request: Request) -> RolloutResponse:
    ...
    for module in modules:
        ...
        response_text, response_json, raw_tool_calls = await call_chat_completion(
            policy_config,
...
def build_config() -> TaskAppConfig:
    ...
    async def startup_http_client(app: Any) -> None:
        try:
            ...
            print("[heartdisease_3step] Created app-level aiohttp client session singleton", flush=True)
        except ImportError:
            try:
                import httpx
                ...
                print("[heartdisease_3step] Created app-level httpx client singleton (fallback)", flush=True)
            except Exception as exc:
                print(f"[heartdisease_3step] WARNING: Failed to create http client: {exc}", flush=True)
                app.state.http_client = None
        except Exception as exc:
            print(f"[heartdisease_3step] WARNING: Failed to create aiohttp client: {exc}", flush=True)
            app.state.http_client = None
    ...
    async def shutdown_http_client(app: Any) -> None:
        ...
        if http_client is not None:
            try:
                ...
                print("[heartdisease_3step] Closed app-level http client", flush=True)
            except Exception as exc:
                print(f"[heartdisease_3step] WARNING: Error closing http client: {exc}", flush=True)
    ...
    return TaskAppConfig(
        ...
        proxy=proxy_config, routers=(heartdisease_router,),
