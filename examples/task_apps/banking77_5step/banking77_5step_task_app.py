...
def build_config() -> TaskAppConfig:
    ...
    async def startup_http_client(app: Any) -> None:
        try:
            ...
            print("[banking77_5step] Created app-level aiohttp client session singleton", flush=True)
        except ImportError:
            try:
                import httpx
                ...
                print("[banking77_5step] Created app-level httpx client singleton (fallback)", flush=True)
            except Exception as exc:
                print(f"[banking77_5step] WARNING: Failed to create http client: {exc}", flush=True)
                app.state.http_client = None
        except Exception as exc:
            print(f"[banking77_5step] WARNING: Failed to create aiohttp client: {exc}", flush=True)
            app.state.http_client = None
    ...
    async def shutdown_http_client(app: Any) -> None:
        ...
        if http_client is not None:
            try:
                ...
                print("[banking77_5step] Closed app-level http client", flush=True)
            except Exception as exc:
                print(f"[banking77_5step] WARNING: Error closing http client: {exc}", flush=True)
