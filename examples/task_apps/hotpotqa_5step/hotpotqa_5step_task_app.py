...
def build_config():
    ...
    async def startup_http_client(app: Any) -> None:
        try:
            ...
            print("[hotpotqa_5step] Created app-level aiohttp client session singleton", flush=True)
        except ImportError:
            try:
                import httpx
                app.state.http_client = httpx.AsyncClient(timeout=30.0, limits=httpx.Limits(max_keepalive_connections=5, max_connections=10))
                print("[hotpotqa_5step] Created app-level httpx client singleton (fallback)", flush=True)
            except Exception as exc:
                print(f"[hotpotqa_5step] WARNING: Failed to create http client: {exc}", flush=True)
                app.state.http_client = None
        except Exception as exc:
            print(f"[hotpotqa_5step] WARNING: Failed to create aiohttp client: {exc}", flush=True)
            app.state.http_client = None
    ...
    async def shutdown_http_client(app: Any) -> None:
        ...
        if http_client is not None:
            try:
                ...
                print("[hotpotqa_5step] Closed app-level http client", flush=True)
            except Exception as exc:
                print(f"[hotpotqa_5step] WARNING: Error closing http client: {exc}", flush=True)
    ...
    base_info = TaskInfo(task={"id": "hotpotqa_5step", "name": "HotpotQA Five-Step Pipeline"}, environment="hotpotqa_5step",
                        ...
                        task_metadata={"modules": [PARSER_MODULE, RETRIEVER_MODULE, REASONER_MODULE, SYNTHESIZER_MODULE, ANSWERER_MODULE]},
