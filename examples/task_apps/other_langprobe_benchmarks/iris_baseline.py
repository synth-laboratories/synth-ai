#!/usr/bin/env python3
"""Baseline script for Iris task app with gpt-5-nano.

This script runs a baseline evaluation on the Iris task app using gpt-5-nano
...
async def run_baseline(
    ...
    async with httpx.AsyncClient(timeout=300.0) as client:
        ...
        try:
            ...
            print("Make sure the task app is running: python -m examples.task_apps.other_langprobe_benchmarks.iris_task_app")
        
        print(f"Running baseline evaluation with {model} on {len(seeds)} seeds...")
        
        for seed in seeds:
            rollout_request = {
                "run_id": f"iris_baseline_{model}_{seed}",
                "env": {
                    "env_name": "iris",
                    "seed": seed,
                    "config": {
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Run Iris baseline with gpt-5-nano")
    parser.add_argument(
        "--task-app-url",
        default="http://127.0.0.1:8115",
        help="URL of the Iris task app",
    )
    parser.add_argument(
        "--seeds",
        type=str,
