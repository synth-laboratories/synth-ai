#!/usr/bin/env python3
"""Baseline script for HeartDisease task app with gpt-5-nano.

This script runs a baseline evaluation on the HeartDisease task app using gpt-5-nano
...
async def run_baseline(
    ...
    """Run baseline evaluation on HeartDisease task app.
    ...
    async with httpx.AsyncClient(timeout=300.0) as client:
        ...
        try:
            ...
            print("Make sure the task app is running: python -m examples.task_apps.other_langprobe_benchmarks.heartdisease_task_app")
        
        print(f"Running baseline evaluation with {model} on {len(seeds)} seeds...")
        
        for seed in seeds:
            rollout_request = {
                "run_id": f"heartdisease_baseline_{model}_{seed}",
                "env": {
                ...
                "policy": {
                    "policy_name": "heartdisease_baseline",
                    "config": {
                        "model": model,
                ...
                if trajectories:
                    ...
                    if steps:
                        ...
                        results.append({
                            ...
                            "correct": is_correct,
                            "reward": reward,
                            "expected": info.get("expected_label", ""),
                            "predicted": info.get("predicted_label", ""),
                        })
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Run HeartDisease baseline with gpt-5-nano")
    parser.add_argument(
        "--task-app-url",
        default="http://127.0.0.1:8114",
        help="URL of the HeartDisease task app",
    )
    parser.add_argument(
        "--seeds",
        type=str,
