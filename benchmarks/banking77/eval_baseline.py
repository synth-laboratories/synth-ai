                ...
                if "choices" in result and len(result["choices"]) > 0:
                    ...
                    if "message" in choice and "tool_calls" in choice["message"]:
                        ...
                        if is_correct:
                            correct += 1
                        total += 1
                        print(f"[{seed:3d}] {'✓' if is_correct else '✗'} pred={predicted[:30]:30s} exp={expected_lower[:30]}")
                    else:
                        print(f"[{seed:3d}] ✗ No tool call in response")
                        total += 1
                elif "error" in result:
                    print(f"[{seed:3d}] ✗ API error: {result['error'].get('message', result['error'])}")
                    total += 1
                else:
...
async def main():
    ...
    parser.add_argument("--seeds", default="100-119", help="Seed range (e.g., 100-119 or 100,105,110)")
