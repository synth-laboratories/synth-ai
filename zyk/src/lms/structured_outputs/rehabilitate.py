...
import re
import logging
from zyk.src.lms.vendors.core.openai_api import OpenAIStructuredOutputClient
...
def fix_errant_stringified_json_sync(response_raw: str, response_model: Type[BaseModel], models: List[str] = ["gpt-4o-mini","gpt-4o"]) -> BaseModel:
    try:
        ...
        for model in models:
            if model == "gpt-4o":
                print("Warning - using gpt-4o for structured output correction - this could add up over time (latency, cost)")
            try:
                fixed_response = mini_client._hit_api_sync(model, messages = messages, lm_config = {"temperature": 0.0})
