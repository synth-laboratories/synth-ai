...
from zyk.src.lms.structured_outputs.rehabilitate import (
            ...
            #         break
                ...
                try:
                    ...
                    structured_output = await fix_errant_forced_async(
                        messages_with_json_formatting_instructions, raw_text_response,
                        response_model,
                        "gpt-4o-mini",
                    )
        ...
        while remaining_retries > 0:
            ...
            #         break
                ...
                try:
                    #t0 = time.time()
                    #print(f"Got error {e}, attempting to fix")
                    structured_output = fix_errant_forced_sync(raw_text_response, response_model, "gpt-4o-mini")
                    break
                    #print(f"Time to fix: {time.time() - t0}")
                except Exception as e:
                    previously_failed_error_messages.append(
...
class ForcedJSONHandler(StructuredHandlerBase):
    ...
    def _process_call_sync(
        ...
        assert response_model is not None, "Don't use this handler for unstructured outputs"
        return api_call_method(messages=messages, model=model, response_model=response_model, temperature=temperature, use_ephemeral_cache_only=use_ephemeral_cache_only)
