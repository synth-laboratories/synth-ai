        ...
        for key, claim in self.claims.items():
            ...
            if key in self.superseded_claims:
                ...
                for old_claim in self.superseded_claims[key]:
                    if old_claim.created_at <= at_time:
                        if old_claim.superseded_at is None or old_claim.superseded_at > at_time:
    ...
    def expand_from_seeds(
        ...
        for _ in range(hops):
            ...
            for subj in frontier:
                for other, weight, _ in self.get_related_subjects(subj, min_weight):
...
def simulate_gameplay_session():
    ...
    print("\n  Evidence details:")
    for ev_id, ev in observer.record.evidence.items():
