...
try:
    ...
    # Go up to langprobe directory: banking77 -> task_specific -> langprobe
...
async def run_gepa_ai_banking77(
    ...
    # Store adapter reference for later access (outside the with block)
    budget_tracking_adapter_ref = [None]
    ...
    with GEPALogger(str(gepa_log_file)):
        ...
        # Wrap adapter to enforce hard budget limit on actual metric calls
        # GEPA's max_metric_calls counts example evaluations, not metric calls
        # We need to track actual adapter.evaluate() calls
        
        class BudgetTrackingAdapter:
            """Wrapper that tracks actual metric calls (adapter.evaluate() invocations)."""
            def __init__(self, adapter, max_metric_calls: int):
                self._adapter = adapter
                self.max_metric_calls = max_metric_calls
                self.metric_call_count = 0
                self._budget_exceeded = False
            
            def evaluate(self, inputs, candidate, **kwargs):
                """Track metric calls and enforce hard budget limit."""
                if self._budget_exceeded:
                    raise RuntimeError(
                        f"Hard budget limit exceeded: {self.metric_call_count} >= {self.max_metric_calls}. "
                        f"Stopping optimization to prevent further calls."
                    )
                
                if self.metric_call_count >= self.max_metric_calls:
                    self._budget_exceeded = True
                    raise RuntimeError(
                        f"Hard budget limit reached: {self.metric_call_count} >= {self.max_metric_calls}. "
                        f"Stopping optimization."
                    )
                
                self.metric_call_count += 1
                return self._adapter.evaluate(inputs, candidate, **kwargs)
            
            def __getattr__(self, name):
                """Delegate all other attributes to wrapped adapter."""
                return getattr(self._adapter, name)
        
        # Calculate scaled budget for GEPA's soft limit
        # GEPA counts example evals: discovery=2*minibatch_size per proposal, full_val=len(valset) per candidate
        # We want to allow some proposals but enforce hard limit via adapter wrapper
        # Estimate: allow ~10-20 proposals worth of example evals as soft limit
        estimated_proposals = min(20, rollout_budget // (2 * reflection_minibatch_size))
        scaled_budget_for_gepa = estimated_proposals * 2 * reflection_minibatch_size + len(valset) * 5  # Allow ~5 full evals
        
        budget_tracking_adapter = BudgetTrackingAdapter(adapter, max_metric_calls=rollout_budget)
        budget_tracking_adapter_ref[0] = budget_tracking_adapter  # Store reference
        ...
        try:
            result = gepa_optimize(
                ...
                trainset=trainset,
                valset=valset,
                adapter=budget_tracking_adapter,  # Use wrapped adapter
                max_metric_calls=scaled_budget_for_gepa,  # Soft limit for GEPA (in example evals)
            ...
            # Check if it's a budget exceeded error from our wrapper
            if "Hard budget limit" in str(e):
                print(f"\n⚠️  Budget limit reached: {str(e)}")
                print(f"   Metric calls: {budget_tracking_adapter.metric_call_count} / {rollout_budget}")
                # Continue with result if available (might have partial results)
                if result is None:
                    raise
            else:
                raise
        except AssertionError as e:
            # Check traceback to see if it's the pareto_front_valset assertion
            import traceback
            ...
            if is_pareto_bug:
                ...
                raise RuntimeError(
                    ...
                    f"GEPA requires a larger validation set (100+ examples recommended).\n"
                    f"Increase val_seeds range to provide more validation examples."
                ) from e
            else:
                # Other assertion errors from GEPA
                print(f"\n⚠️  GEPA library assertion error: {str(e)}")
                raise RuntimeError(
                    f"GEPA optimization failed: {str(e)}\n"
                    f"Try increasing --rollout-budget or validation set size."
                ) from e
        finally:
            quiet_logger.close()
    ...
    lift = val_score_pct - baseline_val
    lift_pct = lift * 100
    print(f"   Improvement: {lift:+.4f} ({lift_pct:+.1f}%)")
    
    total_time = training_time + final_eval_time
    
    # Calculate accounting metrics - simplified for apples-to-apples comparison
    num_candidates = len(result.candidates) if hasattr(result, "candidates") else 0
    num_full_val_evals = getattr(result, "num_full_val_evals", num_candidates)
    
    # Count discovery evaluations (minibatch evals) - these are training rollouts
    total_discovery_evals = 0
    if hasattr(result, "discovery_eval_counts"):
        total_discovery_evals = sum(result.discovery_eval_counts) if result.discovery_eval_counts else 0
    
    # Total policy rollouts in training = discovery examples evaluated
    # Each discovery eval evaluates minibatch_size examples
    total_training_rollouts = total_discovery_evals
    
    # Per-prompt validation size = len(valset) (validation set size)
    per_prompt_val_size = len(valset)
    
    # Print simplified accounting table for apples-to-apples comparison
    print()
    print("=" * 80)
    ...
    metrics = [
        ("Metric", "Value"),
        ("-" * 40, "-" * 40),
        ("Total Training Rollouts", f"{total_training_rollouts:,}"),
        ("  └─ (policy rollouts during training, excluding final eval)", ""),
        ("Per-Prompt Validation Size", f"{per_prompt_val_size}"),
        ("  └─ (validation examples per candidate evaluation)", ""),
        ("Candidates Evaluated", f"{num_candidates}"),
        ("  └─ (number of prompts/candidates tried)", ""),
        ("Training Time", f"{training_time:.1f}s"),
        ("Final Evaluation Time", f"{final_eval_time:.1f}s"),
        ("Total Time", f"{total_time:.1f}s"),
        ("Baseline Score", f"{baseline_val:.4f} ({baseline_val*100:.1f}%)"),
        ("Final Score", f"{val_score_pct:.4f} ({val_score_pct*100:.1f}%)"),
        ("Lift (improvement)", f"{lift:+.4f} ({lift_pct:+.1f}%)"),
    ]
    
    max_label_len = max(len(row[0]) for row in metrics)
    ...
    # Save learning curve if method exists (stub doesn't have save method)
    if hasattr(learning_curve, 'save'):
